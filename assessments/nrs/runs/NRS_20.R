...
# this layes them out stacked (/)
  psex <- p1/p2/p3
  ggsave(paste0("figs/",thismodname,"_sexratios.png"),plot=psex,width=6,height=4.0,units="in")
  psex
...
M<-modlst[[thismod]]
  ...
  ggsave(paste0("figs/",thismodname,"_FatAge.png"),plot=p1,width=8,height=4.0,units="in")
  ...
  p1/p2
  p3 <-p1/p2
 ggsave(paste0("figs/",thismodname,"_F.png"),plot=p3,width=8,height=8,units="in")
             

  #Compare recruitment, ssb, and bts for all the models
  p1 <- plot_rec(modlst,xlim=c(1975.5,2020.5))
  ggsave(paste0("figs/",thismodname,"_rec.png"),plot=p1,width=8,height=4.0,units="in")
  p1 <- plot_ssb(modlst,xlim=c(1975.5,2020.5),alpha=.1)
  ggsave(paste0("figs/",thismodname,"_ssb.png"),plot=p1,width=8,height=4.5,units="in")
  p1<-plot_bts(modlst) + theme_few(base_size=11)
  ggsave(paste0("figs/",thismodname,"_bts.png"),plot=p1,width=8,height=4.0,units="in")
...
#Plotting just the preferred model (thismod) 
  ...
  ggsave(paste0("figs/",thismodname,"_fsh_sel_early.png"),plot=p1,width=4,height=8,units="in")
 
  p1 <- plot_sel(modlst[[thismod]],title = NULL, alpha = 0.3,styr= 1991,endyr = 2005,bysex = TRUE,sexoverlay = TRUE); p1
  ggsave(paste0("figs/",thismodname,"_fsh_sel_mid.png"),plot=p1,width=4,height=8,units="in")
 
  p1 <- plot_sel(modlst[[thismod]],title = NULL, alpha = 0.3,styr= 2006,endyr = 2020,bysex = TRUE,sexoverlay = TRUE); p1
  ggsave(paste0("figs/",thismodname,"_fsh_sel_recent.png"),plot=p1,width=4,height=8,units="in")
 

  #plot survey selectivity
  p1 <- plot_srv_sel(modlst,themod=thismod, title="Survey selectivity",bysex=TRUE,maxage = 20); p1
  ggsave(paste0("figs/",thismodname,"_bts_sel.png"),plot=p1) #,width=4,height=8,units="in")
  ...
  ggsave(paste0("figs/",thismodname,"_survey_age_comps_1.png"),plot=p1,width=11,height=8.5,units="in")

  p1<-plot_age_comps(modlst[thismod],title="Survey age compositions",type="survey",styr=2001,endyr=2019)
  ggsave(paste0("figs/",thismodname,"_survey_age_comps_2.png"),plot=p1,width=11,height=8.5,units="in")
  
  #plot fishery age comps
  p1<-plot_age_comps(modlst[thismod],title="Fishery age compositions",type="fishery",styr = 1975,endyr = 1999)
  ggsave(paste0("figs/",thismodname,"_fish_age_comps_1.png"),plot=p1,width=11,height=8.5,units="in")

  p1<-plot_age_comps(modlst[thismod],title="Fishery age compositions",type="fishery",styr = 2000,endyr = 2019)
  ggsave(paste0("figs/",thismodname,"_fish_age_comps_2.png"),plot=p1,width=11,height=8.5,units="in")
  
 #plot catches
  p1<-plot_catch(modlst,themod = thismod,obspred= TRUE); p1
  ggsave(paste0("figs/",thismodname,"total_catches_w_pred.png"),plot=p1,width=11,height=8.5,units="in")

  p1<-plot_catch(modlst,themod = thismod,obspred= FALSE); p1
  ggsave(paste0("figs/",thismodname,"total_catches.png"),plot=p1,width=11,height=8.5,units="in")
...
maxFabc<-paste0(round(tier1.df$HM_Fmsyr[tier1.df$Year==endyr+1],3),"   ",round(tier1.df$HM_Fmsyr[tier1.df$Year==endyr+2],3))
