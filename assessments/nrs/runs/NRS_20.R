...
M
  ...
  #mean of Fs -------------------------------------
  p2 <- df.g %>% filter(as.numeric(age)>9) %>% group_by(Year,sex) %>% summarise(Mean=mean(F))          %>%
             ggplot(aes(x=Year,y=Mean,color=sex)) + geom_line(size=2) + theme_few() + ggtitle("Age 10-20 Mean F")
             p1/p2
