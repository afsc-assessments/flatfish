...
#  system(paste0("cd ",.projdir,"; main "))
  ...
  write.csv(t1,"tier3_proj_C.csv",row.names=F)
  ...
  write.csv(t1,"tier3_proj_SSB.csv",row.names=F)
  ...
  write.csv(t1,"tier3_proj_F.csv",row.names=F)
  ...
  write.csv(t1,"tier3_proj_ABC.csv",row.names=F)


  # write some sims out for easy use later...
  bfs <- bf %>% filter(Sim<=30)
  head(bfs)
  write.csv(bfs,"proj.csv")
 # head(bfs)
  ...
  p1 <- pf %>% filter(substr(variable,1,1)=="C",variable!="CStdn",Alt==2) %>% select(Yr,variable,value) %>% spread(variable,value) %>%
    ...
    scale_x_continuous(breaks=seq(thisyr,thisyr+14,2))  +  xlab("Year") + ylim(0,500) + ylab("Tier 3 ABC (kt)") + geom_point() + 
  ...
  p2 <- pf %>% filter(substr(variable,1,1)=="S",variable!="SSBStdn",Alt==2) %>% select(Yr,variable,value) %>% spread(variable,value) %>%
    ...
    scale_x_continuous(breaks=seq(thisyr,thisyr+14,2))  +  xlab("Year") + ylim(0,1500) + ylab("Tier 3 Spawning biomass (kt)") + geom_point() + 
