...
#       called "orig"
  master=paste0(mydir,"/nrs/surveyloss")
...
############4.VARIABLES ####################
`%notin%` <- Negate(`%in%`)
assess_LY=2018          #assessment terminal year
...
6. Summarize results
write.csv(df_res,"nrs_results.csv")
glimpse(df_res)
# Some dumb figure----------
df_res %>% filter(Year==2020) %>% mutate(Endyr=2018+peel,buffer=1-ABC_HM/OFL_AM) %>% 
...
glimpse(df_retro)
df_retro %>% filter(Peel==0)
write.csv(df_retro,"nrs_retros.csv")
