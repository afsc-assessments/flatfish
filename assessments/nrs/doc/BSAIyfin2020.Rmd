...
YFS1=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m1/fm.rep")#this is the accepted model. single natural mortality.
...
catmo=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/data/norpac_age_report_YFS_10_1_20.csv",header=TRUE)

#proportion caught before November 2015-2019 (was 92.4 for 2014-2018)
...
catchZ=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/data/catch_YFS.csv",header=TRUE,skip=1)
...
\fontfamily{cmr}
\fontsize{10}{12}
  Ingrid Spies, Rebecca Haehn, and James Ianelli\\
\selectfont
...
following substantive changes have been made in the BSAI Yellowfin Sole assessment. Due to COVID-19, the `r thisyr` NMFS eastern Bering Sea shelf bottom-trawl survey was not conducted.

#### Changes in the data

   1. The `r thisyr-1` fishery age composition was added.
   2. The `r thisyr-1` survey age composition was added.
   3. Estimates of the retained and discarded portions of the `r thisyr-1` catch were added.
   4. The estimate of the total catch made through the end of `r thisyr+1` was used. Catch of 139,283 t was assumed for the `r thisyr+1` and `r thisyr+2` projections.

### Changes in the assessment methods

Two models were considered in this assessment.

   1.Last year's model, used in the `r thisyr-1` assessment, is referred to as Model 18.1a. The same model updated with `r thisyr` data is referred to as Model 18.1b. Model 18.1a (and 18.1b) use the same natural mortality for males and females, $M$=0.12. 
   
   2. A second model is presented (Model 18.2a) that uses a fixed value for female natural mortality ($M$=0.12) and allows male natural mortality to be estimated within the model. This model was reviewed by the BSAI Plan Team in September, 2020. Model 18.2a is the authors' preferred model. 

## Summary of Results

In `r thisyr` a new model was introduced based on the `r thisyr-1` Model 18.2 that retains female natural mortality fixed at 0.12 while allowing the model to estimate male natural mortality. This model is referred to as Model 18.2a. Model 18.1a (and Model 18.1b) included the survey mean bottom temperature across stations < 100m as a covariate on survey catchability, as in previous years, but added survey start date as an additional covariate within the model, based on a recent study by Nichol et al. (2018). Model 18.2b retains these features.

An unexpected 32% decrease in the NMFS eastern Bering Sea survey biomass was observed in 2018. In 2019 the survey biomass was `r 100*round((YFS2$ob_bts[38]/YFS2$ob_bts[37])-1,2)`% higher than in 2018 at `r formatC(YFS2$ob_bts[38]*1000, format="d",big.mark=",")` t. Spawning biomass estimated by Model 18.2 remained high at `r round(FSB_this/Bmsy_this,2)` * $B_{MSY}$. Therefore, Yellowfin Sole continues to qualify for management under Tier 1a. Similar to recent years, the 1978-2013 age-1 recruitments and the corresponding spawning biomass estimates were used to fit the stock recruitment curve and determine the Tier 1 harvest recommendations. 
...
Catch as of October 6, `r thisyr` was `r catchZ$Total[nrow(catchZ)]` t. Over the past 5 years (`r thisyr-5` - `r thisyr-1`), `r 100*round(prop,4)`% of the catch has taken place by this date. Therefore, the full year's estimate of catch in `r thisyr` was `r formatC(as.numeric(str_replace(catchZ$Total[nrow(catchZ)],",",""))/round(prop,4),format="d",big.mark=",")` t. Future catch for the next 10 years, `r thisyr+1` - `r thisyr+10` was estimated as the mean of the past 10 years catch, `r formatC(cn10,format="d",big.mark=",")` t.
...
Tier	&	1a	&	1a	&	1a	&	1a \\
Projected total (age 6+) biomass (t)	& 2,462,400 t & 2,411,700 t &	`r TotBio_thisF` t	   &	`r TotBio_nextF` t  \\
Projected female spawning biomass (t) &	850,600 t	&	821,500 t	&		`r FSB_thisF` t	         &	`r FSB_nextF` t	 \\
$\:\:\:\:\:\:B_{100\%}$                                 &	1,245,400 t	&	1,245,400 t	&		`r B0_2` t	       &	`r B0_2` t    \\
$\:\:\:\:\:\:B_{MSY\%}$                             &	460,800 t 	&	 460,800 t	&		`r Bmsy_thisF` t	     &	`r Bmsy_nextF   ` t		  \\
...
$OFL$    	                            &	290,000 t	&	284,000 t	&	  `r OFL_thisF` t          &	`r OFL_nextF` t       \\
$maxABC$    	                        &	263,200 t	&	257,800 t	&		`r ABC_thisF ` t      &	`r ABC_nextF ` t   \\
$ABC$ 	                             &	263,200 t	&	257,800 t	&		`r ABC_thisF ` t         &	`r ABC_nextF` t       \\
\hline
...
Yellowfin Sole are distributed in North American waters from off British Columbia, Canada, (approx. lat. 49$^{\circ}$N) to the Chukchi Sea (about lat. 70$^{\circ}$N) and south along the Asian coast off the South Korean coast in the Sea of Japan (to about lat. 35$^{\circ}$N).  Adults exhibit a benthic lifestyle and occupy separate spawning areas in winter and feeding distributions in summer on the eastern Bering Sea shelf (`r figure_nums("spawnplot",display="cite")`).  From over-wintering grounds near the shelf margins, adults begin a migration onto the inner shelf in April or early May each year for spawning and feeding. There appears to be several distinct stocks, although the genetic basis remains to be determined. The stocks are referred to as the Unimak group, the Pribilof-west group, and the Pribilof-east group (`r figure_nums("spawnplot",display="cite")`).  The directed fishery historically occurred from winter through autumn, and NMFS research surveys take place during summer months (Wilderbuer et al. 1992).  Yellowfin Sole are managed as a single stock in the BSAI management area as there is presently no direct evidence of stock structure.
...
Yellowfin Sole have annually been caught with bottom trawls on the Bering Sea shelf since the fishery began in 1954 and were overexploited by foreign fisheries in 1959-62 when catches averaged 404,000 t annually (`r figure_nums("catchfig",display="cite")`, top panel).  As a result of reduced stock abundance, catches declined to an annual average of 117,800 t from 1963-71 and further declined to an annual average of 50,700 t from 1972-77.  The lower yield in this latter period was partially due to the discontinuation of the U.S.S.R. fishery.  In the early 1980s, after the stock condition had improved, catches again increased reaching a peak of over 227,000 t in 1985. Catch of Yellowfin Sole takes place primarily in the eastern Bering Sea, with low levels in the eastern Aleutian Islands (`r figure_nums("catchfig",display="cite")`).
...
Yellowfin Sole are usually headed and gutted, frozen at sea, and then shipped primarily to China and South Korea.  Reprocessed Yellowfin Sole from China may also be sold to Japan, US, and Europe as fillets, among other countries (AFSC 2016).  The first wholesale value of Alaska Yellowfin Sole totaled $97.8 million in 2014.  From 2016 to 2017 the first wholesale price of all Bering Sea flatfish fisheries increased 16% to $192.9 million t and Yellowfin Sole price increased 19% year over year. In 2010, following a comprehensive assessment process, the Yellowfin Sole fishery was certified under the Marine Stewardship Council environmental standard for sustainable and well-managed fisheries.  The certification also applies to all the major flatfish fisheries in the BSAI and GOA.  The total annual catch (t) since 1954 is shown in `r table_nums("catch",display="cite")`. Note that the MFCMA was implemented in 1977.
...
A Markov chain Monte Carlo (MCMC) was performed in ADMB to capture variability in $F_{MSY}$, $B_{MSY}$, recruitment, female spawning biomass, and total (age 1+) biomass. The MCMC was run with 1,000,000 iterations, and thinning every 200. An MCMC was run for each model, 18.1 and 18.1a.
...
In this assessment we consider the model used in the `r thisyr-1` assessment updated with `r thisyr` data. This model is referred to as Model 18.1a. Model 18.1a used the same natural mortality for males and females, $M$=0.12. Last year's model (with `r thisyr-1` data is referred to as Model 18.1. A second model is also considered in this assessment (Model 18.2) that used a fixed value for female natural mortality ($M$=0.12) and allowed male natural mortality to be estimated within the model. Model 18.2 is the preferred model.
...
The SSC determined in December 2006 that Yellowfin Sole would be managed under the Tier 1 harvest guidelines, and therefore future harvest recommendations would be based on maximum sustainable yield $MSY$ and the associated fishing effort $F_{MSY}$ values calculated from a spawner-recruit relationship.  $MSY$ is an equilibrium concept and its value is dependent on both the spawner-recruit estimates which are assumed to represent the equilibrium stock size-recruitment relationship and the model used to fit the estimates.  In the Yellowfin Sole stock assessment model, a Ricker form of the stock-recruit relationship was fit to various combinations of these data and estimates of $F_{MSY}$ and $B_{MSY}$ were calculated, assuming that the fit to the stock-recruitment data represents the long-term productivity of the stock (details provided in Wilderbuer et al. 2018).  

For this assessment, two models were examined. The first was the same model used in the 2018 assessment, Model 18.1a. The second model, Model 18.2, fixed female natural mortality at $M$=0.12 as in previous years, but allowed the model to freely estimate male natural mortality. The model estimated male natural mortality to be higher than female natural mortality, which is in common with known life history parameters of other Alaska flatfish. In Arrowtooth Flounder, higher natural mortality is assumed for males and is consistent with their skewed sex ratio (Wilderbuer and Turnock 2009). Higher natural mortality for male flatfish has been assumed to flatfish from other regions as well (Maunder and Wong 2011).

The two models differ slightly in several parameter estimates. The trend in survey catchability was similar with Model 18.1a and Model 18.2, but the catchability was lower with Model 18.2 (`r figure_nums("plotq",display="cite")`). The sex ratio estimate changed slightly with Model 18.2. The proportion female was estimated to be slightly lower in Model 18.1a than Model 18.2, as higher male natural mortality increased the estimated number of males in the population (`r figure_nums("sexratio",display="cite")`). Overall, the total negative log likelihod was lower for Model 18.2, and provided a better fit to the survey and fishery ages, as well as an improvement to the fit to survey catchability, with the total negative log likelihood reduced from 1,424 in Model 18.1a to 1,356 in Model 18.2 (`r table_nums("liketab",display="cite")`, `r figure_nums("srv_agecomp1",display="cite")`, `r figure_nums("srv_agecomp2",display="cite")`,  `r figure_nums("fsh_agecomp1",display="cite")`,  `r figure_nums("fsh_agecomp2",display="cite")`). 

`r table_nums("Mcomparison",display="cite")` indicates that the ABC values from Model 18.2 for `r thisyr+1` would be `r formatC(1000*(extra_sd2$ABC_HM[1]-extra_sd1$ABC_HM[1]),format="d",big.mark=",")` t higher than Model 18.1a. This is due to the higher biomass estimate resulting from an increased value of male natural mortality in Model 18.2. Model 18.2 also provided a slightly better fit to survey biomass, but this effect was primarily noticeable during the years 1988-1995 . Overall Model 18.2 provided very little change in the fit to survey biomass (`r figure_nums("srvbio_fit",display="cite")`). 

It is important for the Tier 1 calculations to identify which subset of the stock recruitment data is used.  Using the full time series to fit the spawner recruit curve estimates that the stock is most productive at a smaller stock size.  Thus $MSY$ and $F_{MSY}$ are relatively high values and $B_{MSY}$ is a lower value.  If the stock was productive in the past at a small stock size because of non density-dependent factors (environment), then reducing the stock size to low levels could be detrimental to the long-term sustainability of the stock if the environment, and thus productivity, have changed from the earlier period.  Since observations of Yellowfin Sole recruitment at low stock sizes are not available from multiple time periods, it is uncertain if future recruitment events at low stock conditions would be as productive as during the late 1970s.

Given the uncertainty of the productivity of Yellowfin Sole at low spawning stock sizes, and because the AFSC policy for reference point time-series selection is to use the post 1977 regime shift values unless there is a compelling reason to do otherwise, the productivity of Yellowfin Sole in this assessment was estimated by fitting the 1977-`r thisyr-6` spawner-recruit data in the model. The resulting stock rescruitment curves are very similar for Model 18.1a and 18.2 (`r figure_nums("ricker1",display="cite")` and `r figure_nums("ricker2",display="cite")`).  
...
Given these results, Model 18.2 is the preferred model for estimating the Yellowfin Sole stock size and management quantities for the `r thisyr+1` fishing season.  
...
Overall the Environmental/Ecosystem considerations for Yellowfin Sole are Level 1 – Normal: “No apparent environmental/ecosystem concerns.” The main points are summarized below:
...
* Scenario 3:  In all future years, the upper bound on $F_{ABC}$ is set at F60%. (Rationale: This scenario
provides a likely lower bound on $F_{ABC}$ that still allows future harvest rates to be adjusted
downward when stocks fall below reference levels.)
* Scenario 4:  In all future years, $F$ is set equal to the 2014-`r thisyr` average $F$.  (Rationale:  For some stocks, $TAC$ can be well below ABC, and recent average F may provide a better indicator of $F_{TAC}$ than $F_{ABC}$.)
...
Wilderbuer, T. K. D. G. Nichol, and J. Ianelli.  2018.  Yellowfin sole.  In Stock Assessment and Fishery Evaluation Document for Groundfish Resources in the Bering Sea/Aleutian Islands Region as Projected for 2011, chapter 4.  North Pacific Fishery Management Council, P. O. Box 103136, Anchorage, Ak  99510.
...
# Tables
#updated2020
`r table_nums(name = "catch", caption = paste("Foreign and domestic datch (t) of Yellowfin Sole 1954-",thisyr,".  Foreign catches are designated as joint venture processing (JVP) and domestic annual processing (DAP). Catch for ",thisyr," was downloaded October 6, 2020.",sep=""))`
...
\pagebreak
`r table_nums(name = "retdis", caption = paste("Estimates of retained and discarded (t) Yellowfin Sole caught in Bering Sea fisheries from 1991 through September 25th, ",thisyr,", and the proportion discarded.",sep=""))`
...
rowz=c("Greenland Turbot","Other Species","Pacific Cod","Alaska Plaice","Atka Mackerel",
...
\pagebreak
`r table_nums(name = "wtagetab", caption = "Mean weight at age (grams) for Yellowfin Sole (unsmoothed), females presented first, followed by males.")`
```{r wtagetab}
thisyr=2019
...
\pagebreak
`r table_nums(name = "Srvbio_CI", caption = "Yellowfin Sole biomass estimates (t) from the annual Bering Sea shelf bottom trawl survey, with upper and lower 95% confidence intervals, based on Model 18.2.")`
...
\pagebreak
`r table_nums(name = "biomCI", caption = paste("Model estimates of Yellowfin Sole age 2+ total biomass (t) from the",thisyr-1," and ",thisyr," stock assessments, Model 18.1, Model 18.1a, and 18.2."))`
...
```{r natagesrv}
catage_p=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/YFS_EBS_survey/yfs_agecomp_ebs_plusnw.csv",header=TRUE)
catage_s=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/YFS_EBS_survey/yfs_agecomp_ebs_standard.csv",header=TRUE)
...
```{r occurrence}
haul_oto=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/Hauls_otoliths.csv",header=TRUE)
...
rescatch=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/ResearchCatch.csv",header=TRUE)
...
`r table_nums(name="Mcomparison",caption="Comparison of reference points for Model 18.2, 18.1a, and Model 18.1. Values are in metric tons (t).")`
...
$\:\:\:\:\:\:B_{100\%}$                           &  `r B0_2`  &  `r B0_2`  &  `r B0_1`  & `r B0_1` &   1,245,400  & 1,245,400  \\
$\:\:\:\:\:\:B_{MSY\%}$                            &  `r formatC(1000*extra_sd2$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*extra_sd2$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*extra_sd1$Bmsy[1],format="d",big.mark=",")`  & `r formatC(1000*extra_sd1$Bmsy[1],format="d",big.mark=",")`  &  460,800  & 460,800 \\
...
\pagebreak
`r table_nums(name = "FSBCI", caption = paste("Model estimates of Yellowfin Sole female spawning biomass (FSB) in metric tons (t) and upper (HCI) and lower (LCI) 95% confidence intervals from the",thisyr-1," and ",thisyr," stock assessments, including Model 18.1, 18.1a and 18.2."))`
...
rownames(biom)=seq(1954,2020,1)
kable(biom,"latex",booktabs=T,linesep="",format.args=list(format="d",big.mark=","),digits=0,align="r",longtable=TRUE)%>%add_header_above(c(" "=1,"Model 18.2"=3,"Model 18.1a"=3,"Model 18.1 (for 2018)"))
```
...
\pagebreak
`r table_nums(name = "ABCOFL", caption = paste("Yellowfin Sole TAC, OFL, and ABC levels, 1980-2019. Data is in metric tons. Estimates for 2019 are calculated using Model 18.2."))`
```{r ABCOFL, message=FALSE,echo=FALSE}
ABCOFL=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/YFS_ABCOFL.csv",header=TRUE)
...
Incidental catch of FMP Groundfish in theYellowfin Sole fishery. Source: NMFS AKRO Blend/Catch Accounting System; 1991-present."))`
```{r bycatch1, message=FALSE,echo=FALSE}
#bycatch1=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/FMP Groundfish BycatchYFIN.csv",header=TRUE)
...
#  b1[i,j]=sum(bycatch1$Catch..mt.[which(bycatch1$Year==yrs[i]&byc#atch1$Species.Group.Name==spp[j])])
# }
#}
...
#colnames(b1f)=spp
#rownames(b1f)=yrs
#write.csv(b1f,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/b1f.csv")
b1fa=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/b1fa.csv",header=TRUE)
...
b1fb=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/b1fb.csv",header=TRUE)
#colnames(b1fb)=c("Year","Northern RF","Other Flatfish","Other RF","Pacific Cod","Pollock","Rock Sole","Sablefish","Sharpchin/Northern RF","SR/RE/Sharpchin/N. RF")
...
\pagebreak
`r table_nums(name = "bycatch2", caption = paste("Bycatch of Other Species Bycatch in the Yellowfin Sole directed fishery, which includes Octopus, Shark, Skate, Squid, and Sculpin. These species are included in the FMP but not available by species in the FMP Groundfish Incidental catch table. Bycatch reported in metric tons. Source: NMFS AKRO Catch Accounting System, Nontarget Estimates; available for 2003 and later."))`
```{r bycatch2, message=FALSE,echo=FALSE}
bycatch2=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/FMP Other Species Bycatch CAS2_YFIN.csv",header=TRUE)
...
colnames(b2f)=spp
rownames(b2f)=yrs
kable(b2f,"latex",booktabs="TRUE",linesep="",align="r")


```

\pagebreak
`r table_nums(name = "bycatch3", caption = paste("Catch and bycatch (t) of other BSAI target species in the Yellowfin Sole directed fishery from 1992-2019 estimated from a combination of regional office reported catch and observer sampling of the catch. Source:  NMFS AKRO Catch Accounting System, Nontarget Estimates; available for 2003 and later."))`
```{r bycatch3, message=FALSE,echo=FALSE}
#bycatch3=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/Nontarget Species BycatchYFIN.csv",header=TRUE)
...
#for(i in 1: length(yrs)){
# for(j in 1:length(spp)){
#  b3[i,j]=sum(bycatch3$Nontarget.Estimate..mt.[which(bycatch3$Year==yrs[i]&bycatch3$Species.Group.Name==spp[j])])
# }
#}
...
#colnames(b3f)=spp
#rownames(b3f)=yrs
#write.csv(b3f,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/b3f.csv")
b3f=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/b3f.csv",header=TRUE)
...
eco=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/EcoYFS.csv",header=TRUE,skip=1)
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.495\textwidth]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/NBS_survey/yfs10bw.png}
    \includegraphics[width=0.495\textwidth]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/NBS_survey/yfs17bw.png}
    \includegraphics[width=0.495\textwidth]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/NBS_survey/yfs18bw.png}
    \includegraphics[width=0.495\textwidth]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/NBS_survey/yfs19bw.png}
\end{figure}
...
```
`r figure_nums(name = "NBSEBSlens", caption ="Estimated numbers at length (mm) of Yellowfin Sole from the Eastern Bering Sea and the Northern Bering Sea surveys, 2017 and 2019.")`
...
\begin{center}
\includegraphics[width=5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/Yellowfin Sole Spawning Distribution-B.pdf}
\end{center}
...
`r figure_nums(name="obs_sizecomp",caption = paste("Size composition of the Yellowfin Sole catch in 2019 (through mid-September), by subarea and total."))`
...
#fishmo=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/2019fisherylocations.csv",header=TRUE)
...
#ggmap(testmap) + geom_point(aes(x= Longitude, y=Latitude, size=Weight), alpha=0.5, data=fishmo1,colour="red") + facet_wrap((~month))+ xlab("Longitude") +ylab("Latitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) +scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_2019bymo2.png")

```
\begin{center}
\includegraphics[width=6.5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsYFSmo19.png}
\end{center}

`r figure_nums(name="fishmo",caption="Catch of Yellowfin Sole in the BSAI in 2019 by month, reported by observers. Circles represent presence of Yellowfin Sole the catch by the following gear types: non-pelagic trawl, pair trawl, or pelagic trawl.")`
...
```
`r figure_nums(name="cpueline",caption="Catch per unit effort on NMFS eastern Bering Sea surveys, 1982-2019. Units are in kg/km2.")`

\pagebreak
```{r cpuefig,message=FALSE,echo=FALSE}
# this was an oracle search for catch of YFS (spp code = 140)
#from OBSINT debriefed
#NMFS area between 500 and 540 in EBS and AI
#by(yfs$WEIGHT_POSTED[which(yfs$Year==2018)],yfs$AGENCY_GEAR_CODE[which(yfs$Year==2018)],sum) mostly caught by NPT non pelagic trawl 128768t out of 131543.7 t
#by(yfs$WEIGHT_POSTED[which(yfs$Year==2018)],yfs$AGENCY_GEAR_CODE[which(yfs$Year==2018)],sum)
#yfs=data.frame(read.csv("/Users/ingridspies/YFS_Blend_CAS.csv",header=TRUE,as.is=TRUE))
#1996 and 1997 only years with BTR early years 1991-1995 had TRW (plus 1 in 2005 8 in 2010)
...
#catchNPTBTRRTW=matrix(0,length(Areas),length(Yrs))
#for(i in 1:length(Areas)){
#  for(j in 1:length(Yrs)){
#    catchNPTBTRRTW[i,j]=sum(yfs$WEIGHT_POSTED[which(yfs$ADFG_STAT_AREA_CODE==Are#as[i]&yfs$Year==Yrs[j]&yfs$AGENCY_GEAR_CODE%in%c("NPT","BTR","TRW"))],na.rm=TRUE#)
#  }
#}
#colnames(catchNPTBTRRTW)=Yrs
#rownames(catchNPTBTRRTW)=Areas

#catch.csv is for all gear
#catchNPTBTRRTW is all trawl gear
#write.csv(catchNPTBTRRTW,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/catchNPTBTRRTW.csv")

#yrvec=rep(Yrs,each=length(Areas))
#areavec=rep(Areas,length(Yrs))
##catchTRAWL=data.frame(cbind(yrvec,areavec,as.vector(t(catchNPTBTRRTW))))
#colnames(catchTRAWL)=c("Year","ADFG_STAT_AREA_CODE","Catch")
#Longitude=getLongAdfg(catchTRAWL$ADFG_STAT_AREA_CODE)
#Latitude=getLatAdfg(catchTRAWL$ADFG_STAT_AREA_CODE)

#catchTRAWL1=data.frame(cbind(catchTRAWL,Longitude,Latitude))

#write.csv(catchTRAWL1,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/catchTRAWL1.csv")
#catchYFS=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CatchYFS1.csv",header=TRUE)
#catchTRAWL1=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CatchTRAWL1.csv",header=TRUE)
...
# all years trawl only
#catchTRAWL1$Catch=as.numeric(as.character(catchTRAWL1$Catch))
#catchTRAWL2=catchTRAWL1[which(catchTRAWL1$Catch>0),]

#ggmap(testmap) + geom_point(data=catchTRAWL2,aes(x= Longitude, y=Latitude,colour=Catch), alpha=0.5,size=1.0) + 
#scale_color_gradientn(colours=terrain.colors(10))+facet_wrap(~Year)+ylab("Latitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) + scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsTRAWL5.png")

#by(catchTRAWL2$Catch,catchTRAWL2$Year,sum)
#by(catchYFS$Catch,catchYFS$Year,sum)

#catchTRAWL2=catchTRAWL1[which(catchTRAWL1$Catch>0),]
#ggmap(testmap) + geom_point(data=catchTRAWL2,aes(x= Longitude, y=Latitude, fill=Catch), alpha=0.5) + scale_colour_gradient(low = "yellow", high = "red",
#  space = "Lab", na.value = "grey50", guide = "colourbar",
#  aesthetics = "colour")+facet_wrap((~Year))+ xlab("Longitude") +ylab("Latitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) + scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsTRAWL4.png")

#catchYFS=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/ingrids_yfs_query_20191118_v2_output.csv")

#catchYFS1=catchYFS[which(catchYFS$GEAR_TYPE_DESCRIPTION!="Longliner"&catchYFS$GEAR_TYPE_DESCRIPTION!="Pot or Trap"),]

#ggmap(testmap) + geom_point(data=catchYFS1,aes(x= LONDD_END, y=LATDD_END,colour=TOTAL_WEIGHT_KG), alpha=0.5,size=0.5) + 
#scale_color_gradientn(colours=terrain.colors(10))+facet_wrap(~YEAR)+ylab("Latitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) + scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsYFS3.png")

#ggmap(testmap) + geom_point(data=catchYFS1,aes(x= LONDD_END, y=LATDD_END), alpha=0.5,size=0.5,colour="red")+facet_wrap(~YEAR)+ylab("Latitude")+xlab("Longitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) + scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsYFS4.png")

#catchYFS19=catchYFS[which(catchYFS$YEAR==2019&catchYFS$TOTAL_WEIGHT_KG>0&catchYFS$GEAR_TYPE_DESCRIPTION!="Longliner"&catchYFS$GEAR_TYPE_DESCRIPTION!="Pot or Trap"),]
#catchYFS19$MONTH = factor(catchYFS19$MONTH, levels=month.name)

#ggmap(testmap) + geom_point(data=catchYFS19,aes(x= LONDD_END, y=LATDD_END), alpha=0.5,size=0.5,colour="red")+facet_wrap(~Month)+ylab("Latitude")+xlab("Longitude")+theme_bw(base_size=11) +scale_x_continuous(limits = c(-190.5, -155.0), expand = c(0, 0)) + scale_y_continuous(limits = c(50.50, 67.0), expand = c(0, 0))+theme(axis.text.x=element_text(angle=45,hjust=1)) 
#ggsave("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsYFSmo19.png")



```
\begin{center}
\includegraphics[width=7.5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/CPUE_YFS_allyrsYFS4.png}
\end{center}

`r figure_nums(name='cpuefig',caption="Catch of Yellowfin Sole in the BSAI, 1991-2019, by year, reported by observers. Circles represent presence of Yellowfin Sole the catch by the following gear types: non-pelagic trawl, pair trawl, or pelagic trawl.")`
...
```
\begin{center}
    \includegraphics[width=6.5in]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/yfs_cpuediff_2019_arcmap.pdf}
\end{center}
...
```
`r figure_nums(name="srvbio_fit",caption = paste("NMFS eastern Bering Sea survey biomass estimates, with 95% confidence intervals and Model 18.1a and Model 18.2 fit to survey biomass estimates,  from 1982-",thisyr,".",sep=""))`
...
```
`r figure_nums(name="BioFSBCI",caption = paste("Model estimates of total (age 2+) and female spawning biomass with 95% confidence intervals, 1954-",thisyr,", Model 18.2. Dots indicate projection model estimates for 2020 and 2021.",sep=""))`
...
`r figure_nums(name="fishingavgF",caption=paste("Projected female spawning biomass for ",thisyr," to ",thisyr+13," (blue line), with 5% and 95% confidence intervals, and fishing at the 5-year (2014-2018) average fishing mortality rate, F=",round(mean(YFS2$F_m[61:65,18]),4),", Model 18.2."))`
...
ggplot(data=recf)+geom_bar(aes(x=Year,y=Recruitment),alpha=0.4,stat="identity")+geom_hline(yintercept=mean(YFS2$natage_f[,5]+YFS2$natage_m[,5]))+theme_bw()+scale_x_continuous(breaks = seq(1955,2019, by = 2))+theme(axis.text.x=element_text(angle=45,hjust=1))+ylab("Recruitment, age 5, billions of fish")
```
`r figure_nums(name="recfig",caption=paste("Year class strength of age 5 Yellowfin Sole estimated by the stock assessment model. The horizontal line represents the average of the estimates from 63 years of recruitment, 1954-2019, Model 18.2."))`
...
text(YFS2$SSB[66,2]-60,YFS2$F_f[66,19]+.005,"2019",cex=0.9)
...
text(extra_sd2$SSB[2]-60,(extra_sd2$Catch_Assump[2]/extra_sd2$GM_Biom[2])+.005,"2021",cex=0.9)


```

`r figure_nums(name="phaseplane",caption = paste("Fishing mortality rate and female spawning biomass from 1975 to ",thisyr," compared to the F35% and F40% control rules, based on Model 18.2.  Vertical line is B35%. Squares indicate estimates for 2019, 2020, and 2021.",sep=""))`
...
recatch2=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2019/data/Noncommercial Fishery CatchYFS.csv",header=TRUE)
