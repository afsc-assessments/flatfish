...
# Set your local directory--------
	mydir <- "C:\\GitProjects\\flatfish\\assessments"  #change to mine.
...
#       called "orig"
	master=paste0(mydir,"\\nrs\\runs")
	if(!dir.exists(master)) print(paste("NOTE: this master directory (",master,") must exist already and contain a subdirectory with base model configurations "))
	setwd(master)
...
# Retrospective sub dir, created within master
	...
	if(!dir.exists(file.path(master, retros))){ 
		dir.create(paste0(master,"\\",retros), showWarnings = FALSE) 
	} 
	
	main_dir<-paste0(master,"\\",retros)	
...
#	fc <- paste0("mkdir -p ",cpto," ; cp orig/* ",cpto)
	fc <-paste0("mkdir ", cpto)
	system(fc)  #IS added "fc"
	#fc2<-paste0("copy orig/* ",cpto)

	setwd(file.path(getwd(),retros,"orig"))
	fc2<-paste0("copy *.* ",cpto) #why won't this line work?
	system(fc2)
