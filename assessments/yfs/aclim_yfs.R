...
mytheme <- mytheme + theme(text=element_blanknt_text(size=18)) + theme(axis.title.x=element_text(size=24) ,axis.title.y=element_text(size=24))
...
# Plot SSB 
  M <- modlst
p <- plot_ssb(M,alpha=.2,xlim=c(1955,2017),ylim=c(0,1400))
catch.df <- read_table2("catch.dat",col_names=TRUE)
catch.df
p <- p + geom_line(data=catch.df,aes(x=Yr,y=Catch)) 
ggplot(catch.df,aes(x=Yr,y=Catch))  + geom_line(size=3,color="blue") + mytheme + ylab("Catch (kt)") + xlab("Year")
M[[1]]
# SRR stuff
source("../R/plot_srr.R")
plot_srr(M[c(1,3)],alpha=.2,xlim=c(0,1100),ylim=c(0,7.2))
plot_srr(M[c(1,6)],alpha=.2,xlim=c(0,1100),ylim=c(0,7.2))
...
plot_ssb(M[c(1,7)],alpha=.2)
plot_bts(M[c(1,7)],alpha=.2)
