 ...
 *(ad_comm::global_datafile) >>  datafile; 
 ...
 if ( phase_mn_rec<0) do_wt_only=1; else do_wt_only=0;
  ...
  alpha_prior.allocate("alpha_prior");
  phase_alpha.allocate("phase_alpha");
  beta_prior.allocate("beta_prior");
  phase_beta.allocate("phase_beta");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  R_logbeta.allocate(phase_sr,"R_logbeta");
  ...
  #endif
  sigma_R.allocate(phase_sigmar,"sigma_R");
  ...
  #endif
  q_like.allocate(1,nsrv,"q_like");
  #ifndef NO_AD_INITIALIZE
    q_like.initialize();
  #endif
  m_like.allocate("m_like");
  #ifndef NO_AD_INITIALIZE
  m_like.initialize();
  #endif
  ...
  #endif
  C_age.allocate(1,nages,"C_age");
  Fmsy.allocate("Fmsy");
  logFmsy.allocate("logFmsy");
  Fmsyr.allocate("Fmsyr");
  logFmsyr.allocate("logFmsyr");
  ABC_biom1.allocate("ABC_biom1");
  ABC_biom2.allocate("ABC_biom2");
  msy.allocate("msy");
  Bmsy.allocate("Bmsy");
  ...
  future_spr0.allocate(styr_fut,endyr_fut,"future_spr0");
  future_catch.allocate(1,num_proj_Fs-1,styr_fut,endyr_fut,"future_catch");
...
void model_parameters::get_2yr_proj(void)
{
    dvar_vector Ntmp_f(1,nages);
    dvar_vector Ntmp_m(1,nages);
...
void model_parameters::report(const dvector& gradients)
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  dvar_vector wt_pop_tmp_f(1,nages);
	...
	nLogPosterior(ilike) = srv_like;   ilike++;
	nLogPosterior(ilike) = catch_like; ilike++;
	nLogPosterior(ilike) = age_like_fsh; ilike++;
	nLogPosterior(ilike) = age_like_srv; ilike++;
	...
	nLogPosterior(ilike) = q_like     ; ilike++;
