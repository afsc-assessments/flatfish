...
_Authors' response: Two models in the current assessment incorporate VAST estimates, one for the EBS(18.3) and one for the EBS+NBS (18.4). Data from the ADF&G survey is presented in this assessment. 
...
m1=ggplot(data=srv_bio4,aes(x=Year,y=Biomass))+geom_ribbon(aes(x=Year,y=Biomass,ymin=LCI,ymax=HCI),alpha=0.2)+geom_line(aes(x=Year,y=Biomass))+geom_line(aes(x=Year,y=Predicted,col=Model))+theme(legend.title=element_text(size=13),legend.text=element_text(size=11),axis.text=element_text(size=11),axis.title.x = element_text(size=10),axis.title.y = element_text(size=14))+
...
```
`r figure_nums(name="mcmcplots",caption="MCMC posterior distributions for Fmsy, Bmsy, log(mean(Recruitment)), Age 6 biomass, female spawning biomass (FSB) for 2020, and total biomass for 2020.")`
