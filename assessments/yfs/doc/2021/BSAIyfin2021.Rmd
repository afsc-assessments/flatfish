...
YFS0=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/fm.rep")#Last year's preferred model is now called 
YFS1=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/fm.rep")#this is single natural mortality.
YFS2=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/fm.rep")#this is the preferred model. Male M is estimated.
YFS3=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/fm.rep")#this is with EBS vast
YFS4=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/fm.rep")#this is with EBS+NBSvast

extra_sd2=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/ABC_OFL.rep",header=TRUE)
extra_sd1=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/ABC_OFL.rep",header=TRUE)
extra_sd3=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/ABC_OFL.rep",header=TRUE)
extra_sd4=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/ABC_OFL.rep",header=TRUE)
extra_sd0=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m21/ABC_OFL.rep",header=TRUE)
...
catchZ=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/catch_YFS.csv",header=TRUE,skip=1)
...
_Changes in the data_
   ...
   4. The `r thisyr` NMFS survey biomass estimate and standard error was included. The `r thisyr` Northern Bering Sea biomass estimate and standard error were combined with the `r thisyr` EBS survey estimate in Model 18.2b.
...
Three models are presented in this assessment. Model 18.2 is presented in full, and is the preferred model. Models 18.3 and 18.4 are presented to promote discussion on the use of VAST biomass estimates and incorporation of the Northern Bering Sea (NBS) survey.
   ...
   2. Model 18.3 is the same as Model 18.2 except it incorporates VAST biomass estimates and standard errors for the Eastern Bering Sea survey region, 1982-2021.
   3. Model 18.4 is the same as Model 18.2 except it incorporates VAST biomass estimates and standard errors for the EBS and NBS, 1982-2021.

## Summary of Results

   
The accepted 2021 Model 18.2 included the survey mean bottom temperature across stations < 100m as a covariate on survey catchability, as well as survey start date as an additional covariate within the model, based on Nichol et al. (2019).  Model 18.2 specifies female natural mortality to be fixed at 0.12 while allowing the model to estimate male natural mortality. 

In the Eastern Bering Sea (EBS) bottom trawl survey performed in `r thisyr`, the Yellowfin Sole biomass in the Eastern Bering Sea was estimated to be `r -100*round((YFS2$ob_bts[39]/YFS2$ob_bts[38])-1,2)`% lower than estimated by the `r thisyr-2` EBS bottom trawl survey, at `r formatC(YFS2$ob_bts[39]*1000, format="d",big.mark=",")` t. Spawning biomass estimated by Model 18.2 remained high at `r round(FSB_this/Bmsy_this,2)` * $B_{MSY}$. Therefore, Yellowfin Sole continues to qualify for management under Tier 1a. The 1978-2014 age-1 recruitments and the corresponding spawning biomass estimates were used to fit the stock recruitment curve and determine the Tier 1 harvest recommendations. 
