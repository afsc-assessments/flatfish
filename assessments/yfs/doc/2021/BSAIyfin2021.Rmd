...
YFS18_2a=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m18_2a/fm.rep")#this is with EBS vast
YFS18_2b=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m18_2b/fm.rep")#this is with EBS+NBSvast
...
ABC_OFL18_2b=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m18_2b/ABC_OFL.rep",header=TRUE)
ABC_OFL2020=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m2020/ABC_OFL.rep",header=TRUE)
B0_2020=formatC(1000*YFS2020$Bzero,format="d",big.mark=",")
B0_18_2=formatC(1000*YFS18_2$Bzero,format="d",big.mark=",")
B0_18_2a=formatC(1000*YFS18_2a$Bzero,format="d",big.mark=",")
B0_18_2b=formatC(1000*YFS18_2b$Bzero,format="d",big.mark=",")

ABC_this=1000*ABC_OFL18_2$ABC_HM[1]
ABC_next=1000*ABC_OFL18_2$ABC_HM[1]
TotBio_this=ABC_OFL18_2$GM_Biom[1]*1000
TotBio_next=ABC_OFL18_2$GM_Biom[2]*1000

femcat=YFS18_2$catage_f/rowSums(YFS18_2$catage_f)
malcat=YFS18_2$catage_m/rowSums(YFS18_2$catage_m)
...
catmo=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2020 YFS/norpac_age_report_YFS_10_1_20.csv",header=TRUE)

#AKFIN groundfish total catch only YFS selected and Bering Sea
YFS_catch1=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/GroundfishTotalCatch_YFS_10_1_21_1991_2021.csv",header=TRUE)


#AKFIN total catch by fishery all fisheries selected and YFS and Bering Sea
YFS_catch=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/GroundfishTotalCatchbyFishery_YFS_10_1_21_1991_2021.csv",header=TRUE)

YFS_catAI=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/GroundfishTotalCatchbyFishery_YFS_AI_10_13_21.csv",header=TRUE)

prop=sum(YFS_catch$Catch..mt.[which(YFS_catch$Month<10&YFS_catch$Year<2021&YFS_catch$Year>2015)],na.rm=TRUE)/sum(YFS_catch$Catch..mt.[which(YFS_catch$Year<2021&YFS_catch$Year>2015)],na.rm=TRUE)
#Between 2016 and 2020, 82.19% of the YFS catch was caught by October 1.
...
Catch[35]=Catch[35]/prop #Estimate 108,087t
...
#####NOTE LOWEST REcruitment in 2012 which was first high temp anomaly year
#Actually there is a lag...
#temprec=as.data.frame(cbind(YFS18_2$yrs_srv[1:38],YFS18_2$natage_f[25:62,1]*1000+YFS18_2$natage_f[25:62,1]*1000,1000*YFS18_2$Bottom_temp[1,1:38]))
#colnames(temprec)=c("Year","Recruitment","Temp_Anomaly*1000")
#library(reshape2)
#text=melt(temprec,id.var="Year")
#ggplot(text)+geom_line(aes(x=Year,y=value,color=variable))+ylab(label="Value")+ggtitle("Numbers at age 1 and bottom temp YFS")+theme_bw()

#low recruitment in 2012 appears to be correlated with warm temps in 2016.
#YFS that were age 4 experienced those warm temps.

#plot(YFS18_2$yrs_srv,YFS18_2$Bottom_temp[1,1:39],type="l")
#plot(YFS18_2$Yr,YFS18_2$natage_f[,1])#recruitment

catchZ=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2021/catch_YFS.csv",header=TRUE,skip=1)

c10=catchZ$BS[(nrow(catchZ)-10):(nrow(catchZ)-1)]
cn=str_replace(c10, ",", "")
cn10=mean(as.numeric(cn))#average catch last 10 years 2011-2020=140,0888 t

#cn5 is the estimate of catch for the past 5 years (2017-2021) including the current years projections
c4=catchZ$BS[(nrow(catchZ)-4):(nrow(catchZ)-1)]
cn4=str_replace(c4, ",", "")
cn5=mean(as.numeric(c(cn4,Catch[35])))#123,080t
...
Relative to last year’s Bering Sea and Aleutian Islands (BSAI) SAFE report, the following substantive changes have been made to the BSAI Yellowfin Sole assessment. Several models are presented in this document that incorporate new data since the last full assessment in `r thisyr-1`.

_Changes in the data_

   1. The `r thisyr-1` fishery age composition was added.
   2. The estimate of the total catch made through the end of `r thisyr-1` was updated as reported by the NMFS Alaska Regional office. The catch through the end of `r thisyr` was estimated based on available data to be `r Catch[35]` t. Catch for the `r thisyr+1` and `r thisyr+2` projections were assumed to be the mean of the past 5 years, `r thisyr-4` - `r thisyr`, `r formatC(cn5,format="d",big.mark=",")` t.
   4. The `r thisyr` NMFS survey biomass estimate and standard error was included. A VAST estimate of the EBS biomass estimate and standard errors were used in Model 18.2a. The `r thisyr` Northern Bering Sea biomass estimate and standard error were combined with the `r thisyr` EBS survey VAST estimate in Model 18.2b.

_Changes in the assessment methods_

Three models are presented in this assessment. Model 18.2 is presented in full, and is the preferred model. Models 18.2a and 18.2b are presented to promote discussion on the use of VAST biomass estimates and incorporation of the Northern Bering Sea (NBS) survey.

   1. Model 18.2 uses a fixed value for female natural mortality ($M$=0.12) and allows male natural mortality to be estimated within the model. This model was accepted by the BSAI Plan Team and the SSC in 2021. Model 18.2 is the authors' preferred model. 
   2. Model 18.2a is the same as Model 18.2 except it incorporates VAST biomass estimates and standard errors for the Eastern Bering Sea survey region, 1982-2021.
   3. Model 18.2c is the same as Model 18.2 except it incorporates VAST biomass estimates and standard errors for the EBS and NBS,combined, 1982-2021. These estimates used all valid NBS survey data (1985, 1988, 1991, 2010, 2017, 2018, 2019, and 2021) and all valid EBS survey data estimates (1982-2021, except 2020).

## Summary of Results

The accepted 2021 Model 18.2 includes survey mean bottom temperature across stations < 100m as a covariate on survey catchability, as well as National Marine Fisheries Service Eastern Bering Sea survey start date as an additional covariate within the model, based correlations documented in Nichol et al. (2019).  Model 18.2 specifies female natural mortality to be fixed at 0.12 while allowing the model to estimate male natural mortality. This model is presented in this year's assessment and is the preferred model.

In the Eastern Bering Sea (EBS) bottom trawl survey performed in `r thisyr`, the EBS Yellowfin Sole biomass was estimated to be `r 100*(1-round((YFS18_2$ob_bts[39]/YFS2020$ob_bts[38]),2))`% lower than estimated by the `r thisyr-2` EBS bottom trawl survey, at `r formatC(YFS2020$ob_bts[39]*1000, format="d",big.mark=",")` t. Spawning biomass estimated by Model 18.2 was `r round(FSB_this/Bmsy_this,2)` * $B_{MSY}$. Therefore, Yellowfin Sole continues to qualify for management under Tier 1a. The 1978-2015 age-1 recruitments and the corresponding spawning biomass estimates were used to fit the stock recruitment curve and determine the Tier 1 harvest recommendations. 

This assessment updates last year’s assessment with results and management quantities that are lower than the `r thisyr-1` assessment. 

Catch as of October 1, `r thisyr` was `r catchZ$Total[nrow(catchZ)]` t. Over the past 5 years (`r thisyr-5` - `r thisyr-1`), `r 100*round(prop,3)`% of the catch has taken place by this date. Therefore, the full year's estimate of catch in `r thisyr` was extrapolated to be `r formatC(as.numeric(str_replace(catchZ$Total[nrow(catchZ)],",",""))/round(prop,4),format="d",big.mark=",")` t. This is lower than the average catch over the past ten years `r formatC(cn10,format="d",big.mark=",")` t.  Future catch for the next 10 years, `r thisyr+1` - `r thisyr+10` was estimated to be the mean of the catch from 2017-2020 and the extrapolated full year's catch for 2021, `r formatC(cn5,format="d",big.mark=",")` t. Catches in 2021 were likely impacted by a 25% tariff on exports to China; therefore, the estimate for future catches is somewhat precautionary.

Yellowfin Sole female spawning biomass continues to be above $B_{MSY}$ and the annual harvest remains below the ABC level. Management quantities are given in the following table for the `r thisyr-1` accepted model (Model 18.2 - 2020) and the `r thisyr` preferred model (Model 18.2 - 2021). The projected estimate of total biomass for `r thisyr+1` was higher by `r (100*(round(TotBio_this/TotBio_last,2)))-100`% from the `r thisyr-1` assessment of `r TotBio_lastF` t, to `r TotBio_thisF` t. The model projection of spawning biomass for `r thisyr+1`, assuming catch for `r thisyr` as described above, was `r FSB_thisF` t, `r 100*round(FSB_this/FSB_last,2)-100`% higher than the projected `r thisyr` spawning biomass from the `r thisyr-1` assessment of `r FSB_lastF` t. The `r thisyr+1` and `r thisyr+2` ABCs using $F_{ABC}$ from this assessment model were higher than the `r thisyr-1` ABC of `r ABC_lastF` t; `r ABC_thisF` t and `r ABC_nextF` t. The `r thisyr+1` and `r thisyr+2` OFLs estimated by model 18.2 were `r OFL_thisF` t and `r OFL_nextF` t.  
...
\begin{tabular}{lrr|rr}
  ...
  \hline
	$M$ (natural mortality rate)	&	0.12, 0.135	&	0.12, 0.135	&	0.12, 0.135	&	0.12, 0.135 \\
Tier	&	1a	&	1a	&	1a	&	1a \\
Projected total (age 6+) biomass (t)	& `r formatC(ABC_OFL2020$GM_Biom[1]*1000,format="d",big.mark=",")` t & `r formatC(ABC_OFL2020$GM_Biom[2]*1000,format="d",big.mark=",")` t &	`r TotBio_thisF` t	   &	`r TotBio_nextF` t  \\
Projected female spawning biomass (t) &	`r formatC(ABC_OFL2020$SSB[1]*1000,format="d",big.mark=",")` t	&	`r formatC(ABC_OFL2020$SSB[2]*1000,format="d",big.mark=",")` t	&		`r FSB_thisF` t	         &	`r FSB_nextF` t	 \\
$\:\:\:\:\:\:B_{100\%}$                                 &	1,275,940 t	&	1,275,940 t	&		`r B0_18_2` t	       &	`r B0_18_2` t    \\
$\:\:\:\:\:\:B_{MSY\%}$                             &	`r formatC(ABC_OFL2020$Bmsy[1]*1000,format="d",big.mark=",")` t	&	 `r formatC(ABC_OFL2020$Bmsy[2]*1000,format="d",big.mark=",")` t	&		`r Bmsy_thisF` t	     &	`r Bmsy_nextF   ` t		  \\
$F_{OFL}$                             &	`r round(ABC_OFL2020$AM_Fmsyr[1],3)`	      &	`r round(ABC_OFL2020$AM_Fmsyr[2],3)`		      &		`r FOFL_this`	   &	`r FOFL_next`	\\
$maxF_{ABC}$                          & `r round(ABC_OFL2020$HM_Fmsyr[1],3)`		      &	`r round(ABC_OFL2020$HM_Fmsyr[2],3)`		      &	  `r FABC_this`	     &	`r FABC_next`  	\\
$F_{ABC}$                             &	`r round(ABC_OFL2020$HM_Fmsyr[1],3)`      &	`r round(ABC_OFL2020$HM_Fmsyr[2],3)`      &		`r FABC_next`           &	`r FABC_next`        \\
$OFL$    	                            &	`r formatC(ABC_OFL2020$OFL_AM[1]*1000,format="d",big.mark=",")` t	&	`r formatC(ABC_OFL2020$OFL_AM[2]*1000,format="d",big.mark=",")` t	&	  `r OFL_thisF` t          &	`r OFL_nextF` t       \\
$maxABC$    	                        &	`r formatC(ABC_OFL2020$ABC_HM[1]*1000,format="d",big.mark=",")` t	&	`r formatC(ABC_OFL2020$ABC_HM[2]*1000,format="d",big.mark=",")` t	&		`r ABC_thisF ` t      &	`r ABC_nextF ` t   \\
$ABC$ 	                             &	`r formatC(ABC_OFL2020$ABC_HM[1]*1000,format="d",big.mark=",")` t	&	`r formatC(ABC_OFL2020$ABC_HM[2]*1000,format="d",big.mark=",")` t	&		`r ABC_thisF ` t         &	`r ABC_nextF` t       \\
\hline
...
Yellowfin Sole (_Limanda aspera_) are one of the most abundant flatfish species in the eastern Bering Sea (EBS) and currently is the target of the largest flatfish fishery in the world. Yellowfin Sole are distributed in North American waters from off British Columbia, Canada, (approx. lat. 49$^{\circ}$N) to the Chukchi Sea (approx. lat. 70$^{\circ}$N) and south along the Asian coast off the South Korean coast in the Sea of Japan (approximately lat. 35$^{\circ}$N). Their abundance in the Aleutian Islands region is considered low to negligible. 

Adults exhibit a benthic lifestyle and occupy separate spawning areas in winter and feeding distributions in summer on the eastern Bering Sea shelf (`r figure_nums("spawnplot",display="cite")`).  Adults begin a migration from over-wintering grounds near the shelf margins (>100m) onto the inner shelf (15-75m) in April or early May each year for spawning and feeding. Adults migrate back offshore in fall and winter as a response to ice cover/cold water of the inner and central shelf water in winter (Bakkala 1979). Young Yellowfin Sole remain in the shallow nearshore nursery areas throughout their first few years of life. They begin to disperse offshore age 3-5, and by 5-8 years they follow adult migratory patterns (Bakkala 1979).

Year-class strength of flatfishes is thought to be determined during the first few years of life between the pelagic egg and benthic settlement (van der Veer et al., 2015). Temperature in the early life stages can affect egg size, larval duration, size at settlement, as well as the size of suitable nursery habitat (Yeung and Cooper 2020). It has been hypothesized that colder bottom temperatures delay migration and spawning in Yellowfin Sole. As a result, mature individuals may reside in nearshore nursery grounds during months in which the NMFS survey occurs, which likely decreases survey biomass estimates during cold years (Nichol et al., 2019; Yeung and Cooper 2020).

YFS may be less sensitive to temperature due to their settlement timing, relative to Northern Rock Sole, which seems to be sensitive to temperature. YFS settle later in summer, when the influence of the cold pool is weaker and nearshore bottom temperature is relatively stable and high (Yeung and Yang, 2018). In contrast, YFS migrate across the shelf to spawn near their nursery habitat, rather than relying on currents for larval transport to nursery habitat (Nichol and Acuna, 2001); therefore, their larvae may be less susceptible to variable currents (Yeung and Cooper 2020).

There appear to be several distinct stocks, although the genetic basis remains to be determined. The stocks are referred to as the Unimak group, the Pribilof-west group, and the Pribilof-east group (`r figure_nums("spawnplot",display="cite")`).  Yellowfin Sole are managed as a single stock in the BSAI management area as there is presently no direct evidence of stock structure.

## Fishery     

Yellowfin Sole has been targeted with bottom trawls on the Bering Sea shelf since the fishery began in 1954. It was overexploited by foreign fisheries in 1959-62 when catches averaged 404,000 t annually (`r figure_nums("catchfig",display="cite")`, top panel).  Catches declined to an annual average of 117,800 t from 1963-1971 and further declined to an annual average of 50,700 t from 1972-1977.  The lower yield in this latter period was partially due to the discontinuation of the Union of Soviet Socialist Republics (U.S.S.R.) fishery. In the early 1980s, after the stock condition had improved, catches again increased reaching a peak of over 227,000 t in 1985. Catch of Yellowfin Sole takes place primarily in the eastern Bering Sea, with low to negligible levels in the eastern Aleutian Islands.
...
For this assessment, two models were examined in full (Model 18.1 and 18.2), and two additional exploratory models were examined, Model 18.2a and 18.4. Model 18.1 was the accepted model in the 2018 and 2019 Yellowfin Sole stock assessments. Model 18.2 fixed female natural mortality at $M$=0.12 as in previous years, but allowed the model to freely estimate male natural mortality. The model estimated male natural mortality to be higher than female natural mortality, which is in common with known life history parameters of other Alaska flatfish. In Arrowtooth Flounder, higher natural mortality is assumed for males and is consistent with their skewed sex ratio (Wilderbuer and Turnock 2009). Higher natural mortality for male flatfish has been assumed to flatfish from other regions as well (Maunder and Wong 2011).

Models 18.1 and 18.2 differ for some parameter estimates. The trend in survey catchability was similar with Model 18.1 and Model 18.2, but catchability was lower with Model 18.2 (`r figure_nums("plotq",display="cite")`). The sex ratio estimate changed slightly with Model 18.2. The proportion female was estimated to be slightly lower in Model 18.1 than Model 18.2, as higher male natural mortality increased the estimated number of males in the population (`r figure_nums("sexratio",display="cite")`). Overall, the total negative log likelihood was lower for Model 18.2, and provided a better fit to the survey and fishery ages, as well as an improvement to the fit to survey catchability, with the total negative log likelihood reduced from 1,449 in Model 18.1 to 1,386 in Model 18.2 (`r table_nums("liketab",display="cite")`, `r figure_nums("srv_agecomp1",display="cite")`, `r figure_nums("srv_agecomp2",display="cite")`,  `r figure_nums("fsh_agecomp1",display="cite")`,  `r figure_nums("fsh_agecomp2",display="cite")`). 

Overall Model 18.2 provided a good fit to survey biomass (`r figure_nums("srvbio_fit",display="cite")`). Models 18.2a and 18.2b fit their corresponding estimates of survey biomass fairly well, but there was some discontinuity in the fit for 2015 that was consistent among models.
...
YFS2021=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_0/fm.rep")
YFS2020=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_1/fm.rep")
YFS2019=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_2/fm.rep")
YFS2018=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_3/fm.rep")
YFS2017=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_4/fm.rep")
YFS2016=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_5/fm.rep")
YFS2015=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_6/fm.rep")
YFS2014=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_7/fm.rep")
YFS2013=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_8/fm.rep")
YFS2012=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_9/fm.rep")
YFS2011=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_10/fm.rep")
a=-1*(YFS2021$SSB[1:67,2]-YFS2020$SSB[,2])/YFS2021$SSB[1:67,2]  
b=-1*(YFS2021$SSB[1:66,2]-YFS2019$SSB[,2])/YFS2021$SSB[1:66,2]
c=-1*(YFS2021$SSB[1:65,2]-YFS2018$SSB[,2])/YFS2021$SSB[1:65,2]
d=-1*(YFS2021$SSB[1:64,2]-YFS2017$SSB[,2])/YFS2021$SSB[1:64,2]
e=-1*(YFS2021$SSB[1:63,2]-YFS2016$SSB[,2])/YFS2021$SSB[1:63,2]
f=-1*(YFS2021$SSB[1:62,2]-YFS2015$SSB[,2])/YFS2021$SSB[1:62,2]
g=-1*(YFS2021$SSB[1:61,2]-YFS2014$SSB[,2])/YFS2021$SSB[1:61,2]
h=-1*(YFS2021$SSB[1:60,2]-YFS2013$SSB[,2])/YFS2021$SSB[1:60,2]
i=-1*(YFS2021$SSB[1:59,2]-YFS2012$SSB[,2])/YFS2021$SSB[1:59,2]
j=-1*(YFS2021$SSB[1:58,2]-YFS2011$SSB[,2])/YFS2021$SSB[1:58,2]
...
A within-model retrospective analysis was included for the recommended assessment model (Model 18.2). In this analysis, retrospective female spawning biomass was calculated by sequentially dropping data one year at a time and then comparing the peeled estimate to the reference stock assessment model used in the assessment (`r figure_nums("retro1",display="cite")`). Mohn's rho for Model 18.2 was `r round(Rho18_2,3)`. A similar retrospective pattern was observed as in recent years, in which earlier retrospective years indicated a lower level of spawning biomass than the current year's data (`r figure_nums("retro1",display="cite")`). The difference in female spawning biomass was negative for most recent years, except for the most recent (`r figure_nums("retro2",display="cite")`), and very similar among models. It is notable that there was very little difference in the retrospective pattern for the current year vs. 2019 and 2018. This is an improvement in the retrospective pattern than seen in previous years. The Mohn’s rho appears to exceed the rule of thumb guideline of 0.20 for long-lived stocks proposed by Hurtado-Ferro at al. (2015), which includes flatfish. The rule of thumb is that values of Mohn’s rho higher than 0.20 or lower than -0.15 may be an indication of a retrospective pattern.
...
The 2019 distribution of age-classes showed older/larger fish over the southern shelf and younger/smaller fish over the northern shelf (L. Britt, pers comm). This indicates favorable growth and survival of juvenile YFS in the NBS. A proposed thermal window (Yeung et al. 2021) suggests continued warming over the EBS shelf may result in temperatures above the thermal physiological maximum of YFS. Such high temperatures in juvenile habitats (i.e., inner domain) could negatively affect production of YFS, which may be adapted to colder temperatures.
...
Bakkala, R.G., 1979. Population characteristics and ecology of yellowfin sole. NWAFC PROCESSED REPORT 79-20, p.280. 
...
Nichol, D. G., and Acuna, E. I. 2001. Annual and batch fecundities of
yellowfin sole, Limanda aspera, in the eastern Bering Sea. Fishery
Bulletin, 99: 108–122.
...
van der Veer, H. W., and Witte, J. I. J. 1993. The ‘maximum growth/optimal food condition’ hypothesis: a test for 0-group plaice _Pleuronectes platessa_ in the Dutch Wadden Sea. Marine Ecology Progress Series, 101: 81–90.
...
Yeung, C., and Yang, M.-S. 2018. Spatial variation in habitat quality
for juvenile flatfish in the southeastern Bering Sea and its implications
for productivity in a warming ecosystem. Journal of Sea Research, 139: 62–72.

Yeung, C. and Cooper, D.W., 2020. Contrasting the variability in spatial distribution of two juvenile flatfishes in relation to thermal stanzas in the eastern Bering Sea. ICES Journal of Marine Science, 77(3), pp.953-963.
...
`r table_nums(name = "catch", caption = paste("Foreign and domestic catch (t) of Yellowfin Sole 1954-",thisyr,".  Foreign catches are designated as joint venture processing (JVP), and non-foreign catches as domestic annual processing (DAP). Domestic catch since 1991 is subdivided into catch from the Aleutian Islands and the Bering Sea. Catch for ",thisyr," was downloaded October 13, 2021.",sep=""))`
...
retdis=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/GroundfishTotalDiscards_EBS_YFS_10_13_21.csv",header=TRUE)#October 13,2021
...
target=names(table(retdis2020$Target))
...
for(i in 1:length(target)){
 ...
 rdtab[i+((j-1)*13),2]=sum(retdis2020$Discards..mt.[which(retdis2020$Target==target[i]&retdis2020$Gear==gear[j])])
 rdtab[i+((j-1)*13),3]=sum(retdis2020$Retained..mt.[which(retdis2020$Target==target[i]&retdis2020$Gear==gear[j])])
 }}
...
```{r occurrence}
haul_oto=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/Hauls_otoliths.csv",header=TRUE)
...
mat=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/maturity_T10.csv",header=TRUE)
...
\pagebreak
`r table_nums(name = "wtagetab", caption = "Mean unsmoothed weight-at-age (grams) for Yellowfin Sole, based on survey data, females presented first, followed by males, 1964-2021, except 2020.")`
```{r wtagetab}
#No new data in 2020
thisyr=2021
...
rownames(wt_f)=seq(1964,thisyr,1)[-57]
colnames(wt_m)=seq(1,20,1)
rownames(wt_m)=seq(1964,thisyr,1)[-57]
...
\pagebreak
`r table_nums(name = "biomCI", caption = paste("Model estimates of Yellowfin Sole age 2+ total biomass (t) from the",thisyr-1," and ",thisyr," stock assessments, Model 18.2, Model 18.2a, and 18.2b."))`
```{r biomCI, message=FALSE,echo=FALSE}
biom=data.frame(round(1000*YFS18_2$TotBiom[,c(2,4,5)]),rbind(round(1000*YFS2020$TotBiom[,c(2,4,5)]),c("-","-","-")),round(1000*YFS18_2a$TotBiom[,c(2)]),round(1000*YFS18_2b$TotBiom[,c(2)]))
colnames(biom)=c("Biomass (t)","LCI","HCI","Biomass (t)","LCI","HCI","Biomass (t)","Biomass (t)")
rownames(biom)=seq(1954,2021,1)
kable(biom,"latex",booktabs=T,linesep="",format.args=list(format="d",big.mark=","),longtable=TRUE,align="r")%>%add_header_above(c(" "=1,"Model 18.2 (2021)"=3,"Model 18.2 (2020)"=3,"Model 18.2a (2021)","MOdel 18.2b (2021"))
```
...
NBS=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/NBS_YFS.csv",header=TRUE)
...
\pagebreak
`r table_nums(name = "natagesrv", caption = paste("Yellowfin Sole population numbers-at-age (millions) estimated from the annual EBS bottom trawl surveys, 1987-",thisyr-2," (Current year data is not yet available and there was no survey in 2020). Data in years 1987 or later come from the 'plusnw' extended survey area. Females are presented first, followed by males.",sep=""))`
```{r natagesrv}
catage_p=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/survey/yellowfin_sole_agecomp_ebs_plusNW.csv",header=TRUE)
catage_p=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/survey/yellowfin_sole_agecomp_ebs_plusNW.csv",header=TRUE)#no new ages since last year.
catage_s=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/yfs_agecomp_ebs_standard.csv",header=TRUE)
...
#Get research catch
rescat=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/NoncommercialFisheryCatch_YFS_10_14_21a.csv")
...
totallike2=sum(YFS18_2$wt_like,YFS18_2$age_likeihood_for_survey,YFS18_2$age_likelihood_for_fishery,YFS18_2$catch_likelihood,YFS18_2$selectivity_likelihood,YFS18_2$survey_likelihood,YFS18_2$recruitment_likelilhood) #recommended
totallike1=sum(YFS18_2a$wt_like,YFS18_2a$age_likeihood_for_survey,YFS18_2a$age_likelihood_for_fishery,YFS18_2a$catch_likelihood,YFS18_2a$selectivity_likelihood,YFS18_2a$survey_likelihood,YFS18_2a$recruitment_likelilhood) 
```
...
\begin{tabular}{ lrr }
\hline
 Likelihood component & Model $18.2$ & Model $18.2a$ \\
\hline
Survey age	        &       `r round(YFS18_2$age_likeihood_for_survey,2)`  &  `r round(YFS18_2a$age_likeihood_for_survey,2)`   \\
Fishery age  	  &       `r round(YFS18_2$age_likelihood_for_fishery,2)` &  `r round(YFS18_2a$age_likelihood_for_fishery,2)`  \\
Selectivity     &       `r round(sum(YFS18_2$selectivity_likelihood),2)`& `r round(sum(YFS18_2a$selectivity_likelihood),2)`\\
Survey biomass     &       `r round(YFS18_2$survey_likelihood,2)`& `r round(YFS18_2a$survey_likelihood,2)`\\
Recruitment     &       `r round(sum(YFS18_2$recruitment_likelilhood),2)`& `r round(sum(YFS18_2a$recruitment_likelilhood),2)`  \\
Catchability & `r round(YFS18_2$catch_likelihood,4)` & `r round(YFS18_2a$catch_likelihood,4)` \\
\hline
...
pars=as.data.frame(read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m18_2/fm.std",header=TRUE))
pars2=pars[c(1:6,114,183,184,321,390,459:464,712:716,736,737,738:805),]
...
`r table_nums(name="Mcomparison",caption="Comparison of reference points for Model 18.2 (2021), 18.2 (2020), and Models 18.2a and 18.2b (lower panel). Values are in metric tons (t). Female, then male natural mortality is listed for each year and model.")`

\begin{table}[ht]
\centering
\begin{tabular}{lrr|rr}
  \hline
       & \multicolumn{2}{c|}{Model 18.2 (2021)}  & \multicolumn{2}{c}{Model 18.2 (2020)}}               \\
        Quantity & `r thisyr+1`      &`r thisyr+2`   & `r thisyr+1`      &`r thisyr+2` \\ 
  \hline
$M$ (natural mortality rate)         &  `r round(YFS18_2$natmort_f,2)`, `r round(YFS18_2$natmort_m,3)`  & `r round(YFS18_2$natmort_f,2)`, `r round(YFS18_2$natmort_m,3)`   &  `r round(YFS2020$natmort_f,2)`, `r round(YFS2020$natmort_m,2)`   &  `r round(YFS2020$natmort_f,2)`, `r round(YFS2020$natmort_m,2)`   \\
Tier                                  &  1a   &  1a   &  1a   & 1a    \\
Projected total (age 6+) biomass (t)  &  `r formatC(ABC_OFL18_2$GM_Biom[1]*1000,format="d",big.mark=",")`   &  `r formatC(ABC_OFL18_2$GM_Biom[2]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL2020$GM_Biom[1]*1000,format="d",big.mark=",")`  & `r formatC(ABC_OFL2020$GM_Biom[2]*1000,format="d",big.mark=",")` \\
Projected female spawning biomass (t) &  `r formatC(ABC_OFL18_2$SSB[1]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL18_2$SSB[2]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL2020$SSB[1]*1000,format="d",big.mark=",")`  & `r formatC(ABC_OFL2020$SSB[2]*1000,format="d",big.mark=",")`  \\
$\:\:\:\:\:\:B_{100\%}$                           &  `r B0_18_2`  &  `r B0_18_2`  &  `r B0_2020`  & `r B0_2020`   \\
$\:\:\:\:\:\:B_{MSY\%}$                            &  `r formatC(1000*ABC_OFL18_2$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL2020$Bmsy[1],format="d",big.mark=",")`  & `r formatC(1000*ABC_OFL2020$Bmsy[1],format="d",big.mark=",")`   \\
$F_{OFL}$                             &  `r round(ABC_OFL18_2$AM_Fmsyr[1],3)`   &  `r round(ABC_OFL18_2$AM_Fmsyr[2],3)`   &  `r round(ABC_OFL2020$AM_Fmsyr[1],3)`  & `r round(ABC_OFL2020$AM_Fmsyr[2],3)`  \\
$maxF_{ABC}$                          &  `r round(ABC_OFL18_2$HM_Fmsyr[1],3)`   &   `r round(ABC_OFL18_2$HM_Fmsyr[2],3)`   &  `r round(ABC_OFL2020$HM_Fmsyr[1],3)`    & `r round(ABC_OFL2020$HM_Fmsyr[2],3)`    \\
$F_{ABC}$                             &  `r round(ABC_OFL18_2$HM_Fmsyr[1],3)`   &  `r round(ABC_OFL18_2$HM_Fmsyr[2],3)`   &  `r round(ABC_OFL2020$HM_Fmsyr[1],3)`   & `r round(ABC_OFL2020$HM_Fmsyr[2],3)`   \\
$OFL$                                 &  `r formatC(1000*ABC_OFL18_2$OFL_AM[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2$OFL_AM[2],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL2020$OFL_AM[1],format="d",big.mark=",")`  & `r formatC(1000*ABC_OFL2020$OFL_AM[2],format="d",big.mark=",")` 
\\
$maxABC$                              &  `r formatC(1000*ABC_OFL18_2$ABC_HM[1],format="d",big.mark=",") ` &  `r formatC(1000*ABC_OFL18_2$ABC_HM[2],format="d",big.mark=",") `  &  `r formatC(1000*ABC_OFL2020$ABC_HM[1],format="d",big.mark=",") `  & `r formatC(1000*ABC_OFL2020$ABC_HM[2],format="d",big.mark=",") `  \\
$ABC$                                 &  `r formatC(1000*ABC_OFL18_2$ABC_HM[1],format="d",big.mark=",") `  &  `r formatC(1000*ABC_OFL18_2$ABC_HM[2],format="d",big.mark=",") ` & `r formatC(1000*ABC_OFL2020$ABC_HM[1],format="d",big.mark=",") `  & `r formatC(1000*ABC_OFL2020$ABC_HM[2],format="d",big.mark=",") `  \\
\hline
...
\begin{tabular}{lrr|rr}
  \hline
       & \multicolumn{2}{c|}{Model 18.2a}  & \multicolumn{2}{c}{Model 18.2b}              \\
        Quantity & `r thisyr+1`      &`r thisyr+2`   & `r thisyr+1`      &`r thisyr+2` \\ 
  \hline
$M$ (natural mortality rate)         &  `r round(YFS18_2a$natmort_f,2)`, `r round(YFS18_2a$natmort_m,3)`   & `r round(YFS18_2a$natmort_f,2)`, `r round(YFS18_2a$natmort_m,3)` &  `r round(YFS18_2b$natmort_f,2)`, `r round(YFS18_2b$natmort_m,3)`   &  `r round(YFS18_2b$natmort_f,2)`, `r round(YFS18_2b$natmort_m,3)`    \\
Tier                                  &  1a   &  1a   &  1a   & 1a \\
Projected total (age 6+) biomass (t)  &  `r formatC(ABC_OFL18_2a$GM_Biom[1]*1000,format="d",big.mark=",")`   &  `r formatC(ABC_OFL18_2a$GM_Biom[2]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL18_2b$GM_Biom[1]*1000,format="d",big.mark=",")`  & `r formatC(ABC_OFL18_2b$GM_Biom[2]*1000,format="d",big.mark=",")`  \\
Projected female spawning biomass (t) &  `r formatC(ABC_OFL18_2a$SSB[1]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL18_2a$SSB[2]*1000,format="d",big.mark=",")`  &  `r formatC(ABC_OFL18_2b$SSB[1]*1000,format="d",big.mark=",")`  & `r formatC(ABC_OFL18_2b$SSB[2]*1000,format="d",big.mark=",")`  \\
$\:\:\:\:\:\:B_{100\%}$                           &  `r B0_18_2a`  &  `r B0_18_2a`  &  `r B0_18_2b`  & `r B0_18_2b`   \\
$\:\:\:\:\:\:B_{MSY\%}$                            &  `r formatC(1000*ABC_OFL18_2a$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2a$Bmsy[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2b$Bmsy[1],format="d",big.mark=",")`  & `r formatC(1000*ABC_OFL18_2b$Bmsy[1],format="d",big.mark=",")` \\
$F_{OFL}$                             &  `r round(ABC_OFL18_2a$AM_Fmsyr[1],3)`   &  `r round(ABC_OFL18_2a$AM_Fmsyr[2],3)`   &  `r round(ABC_OFL18_2b$AM_Fmsyr[1],3)`  & `r round(ABC_OFL18_2b$AM_Fmsyr[2],3)` \\
$maxF_{ABC}$                          &  `r round(ABC_OFL18_2a$HM_Fmsyr[1],3)`   &   `r round(ABC_OFL18_2a$HM_Fmsyr[2],3)`   &  `r round(ABC_OFL18_2b$HM_Fmsyr[1],3)`    & `r round(ABC_OFL18_2b$HM_Fmsyr[2],3)` \\
$F_{ABC}$                             &  `r round(ABC_OFL18_2a$HM_Fmsyr[1],3)`   &  `r round(ABC_OFL18_2a$HM_Fmsyr[2],3)`   &  `r round(ABC_OFL18_2b$HM_Fmsyr[1],3)`   & `r round(ABC_OFL18_2b$HM_Fmsyr[2],3)`  \\
$OFL$                                 &  `r formatC(1000*ABC_OFL18_2a$OFL_AM[1],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2a$OFL_AM[2],format="d",big.mark=",")`  &  `r formatC(1000*ABC_OFL18_2b$OFL_AM[1],format="d",big.mark=",")`  & `r formatC(1000*ABC_OFL18_2b$OFL_AM[2],format="d",big.mark=",")`
\\
$maxABC$                              &  `r formatC(1000*ABC_OFL18_2a$ABC_HM[1],format="d",big.mark=",") ` &  `r formatC(1000*ABC_OFL18_2a$ABC_HM[2],format="d",big.mark=",") `  &  `r formatC(1000*ABC_OFL18_2b$ABC_HM[1],format="d",big.mark=",") `  & `r formatC(1000*ABC_OFL18_2b$ABC_HM[2],format="d",big.mark=",") ` \\
$ABC$                                 &  `r formatC(1000*ABC_OFL18_2a$ABC_HM[1],format="d",big.mark=",") `  &  `r formatC(1000*ABC_OFL18_2a$ABC_HM[2],format="d",big.mark=",") ` & `r formatC(1000*ABC_OFL18_2b$ABC_HM[1],format="d",big.mark=",") `  & `r formatC(1000*ABC_OFL18_2b$ABC_HM[2],format="d",big.mark=",") ` \\
\hline
Status                               & `r thisyr-1`       & `r thisyr`      &  `r thisyr-1`             & `r thisyr`            \\
\hline
Overfishing                           & No         & n/a       & No                        & n/a                 \\
...
\end{tabular}
\begin{tablenotes}
\item Projections for Model 18.2 were based on estimated catches of `r formatC(Catch[35],format="d",big.mark=",")` t in `r thisyr`, and projections for Model 18.2 were based on `r formatC(cn10,format="d",big.mark=",")` t used in place of maximum ABC for `r thisyr+1`. Projections for Models 18.2a and 18.2b were based on estimated catches of `r formatC(cn10,format="d",big.mark=",")` used in place of maximum ABC for `r thisyr+1`.
  \end{tablenotes}
\end{table}
...
fm=data.frame(cbind(YFS18_2$F_f[,18],YFS18_2$Obs_catch/YFS18_2$TotBiom[,2],YFS18_2a$F_f[,18],YFS18_2a$Obs_catch/YFS18_2a$TotBiom[,2]))

colnames(fm)=c("Full selection F","Catch/Total Biomass","Full selection F","Catch/Total Biomass")
rownames(fm)=seq(1954,2021,1)
kable(fm,"latex",booktabs=TRUE,linesep="",align="r",digits=3,longtable=TRUE)%>%add_header_above(c(" "=1,"Model 18.2"=2,"Model 18.2a"=2))
```

\pagebreak
`r table_nums(name = "FSBCI", caption = paste("Model estimates of Yellowfin Sole female spawning biomass (FSB) in the Eastern Bering Sea in metric tons (t) and upper (HCI) and lower (LCI) 95% confidence intervals from the",thisyr-1," and ",thisyr," stock assessments, including Model 18.2 (2020), 18.2 (2021), and 18.2a."))`
```{r FSBCI, message=FALSE,echo=FALSE}
biom=data.frame(1000*YFS18_2$SSB[,c(2,4,5)],rbind(1000*YFS2020$SSB[,c(2,4,5)],c("-","-","-")),1000*YFS18_2a$SSB[,c(2,4,5)])
...
\pagebreak
`r table_nums(name="rec",caption="Model estimates of age 1 recruitment (in billions of fish), 1954-2019, with 95% lower and upper confidence intervals (LCI, HCI) for Model 18.2 (2021) and 18.2 (2020).")`
```{r rec, message=FALSE,echo=FALSE}
pars1=as.data.frame(read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m18_2/fm.std",header=TRUE))
pars2=as.data.frame(read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m2020/fm.std",header=TRUE))

age1recpred1=pars1[546:613,3]
age1recstd1=pars1[546:613,4]

age1recpred2=pars2[539:605,3]
age1recstd2=pars2[539:605,4]

rec=data.frame(age1recpred1,age1recpred1-1.96*age1recstd1,age1recpred1+1.96*age1recstd1,c(age1recpred2,"-"),c(age1recpred2-1.96*age1recstd2,"-"),c(age1recpred2+1.96*age1recstd2,"-"))

rownames(rec)=seq(1954,2021,1)
colnames(rec)=c("Recruitment","LCI","HCI","Recruitment","LCI","HCI")
kable(rec,"latex",booktabs=TRUE,linesep="",align="r",digits=3,longtable=TRUE)%>%add_header_above(c("Year"=1,"Model 18.2 (2020)"=3,"Model 18.2 (2021)"=3))

```


\pagebreak
`r table_nums(name = "ABCOFL", caption = paste("Yellowfin Sole total allowable catch (TAC), overfishing limit (OFL), and acceptable biological catch (ABC) levels, 1980-2021. Catch was recorded through October 1, 2021. Data is in metric tons. Estimates for 2022 were calculated using Model 18.2, and the 2022 TAC has not yet been set."))`
...
bycatch1=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/FMP Groundfish Bycatch1991_2020.csv",header=TRUE)
...
b1fa=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/b1fa.csv",header=TRUE)
...
b1fb=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/b1fb.csv",header=TRUE)
...
bycatch2=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/FMP Other Species Bycatch CAS2_1991-2020.csv",header=TRUE)
...
bycatch3=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/Nontarget Species Bycatch_YFS.csv",header=TRUE)
...
colnames(b3f)=spp
rownames(b3f)=yrs
write.csv(b3f,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/b3f.csv")
b3f=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/b3f.csv",header=TRUE)
...
eco=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/EcoYFS.csv",header=TRUE,skip=1)
...
\begin{center}
\includegraphics[width=5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/Yellowfin Sole Spawning Distribution-B.pdf}
\end{center}
...
catchtab=data.frame(cbind(YFS18_2$Yr,YFS18_2$Obs_catch))
colnames(catchtab)=c("Year","Catch")
catchtab[68,2]=Catch[35]/1000
...
theme_bw()+theme(legend.title=element_text(size=13),legend.text=element_text(size=11),axis.text=element_text(size=9),axis.title.x = element_text(size=13),axis.title.y = element_text(size=14))+
 ylab("Catch (x 1,000 t)")+scale_x_continuous(breaks = seq(1955,2021, by = 5))+theme(legend.position="right")+scale_color_discrete(name="")
```
...
catch_month=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/catch_month.csv")
...
par(mar=c(5,6,6,1))
retdis2020=retdis[which(retdis$Year=="2020"),]
area=names(table(retdis2020$Reporting.Area.Code))
areaz=vector()
for(i in 1:length(area)){
 areaz[i]=sum(retdis2020$Total.Catch..mt.[which(retdis2020$Reporting.Area.Code==area[i])])
}
...
cat_mo2021=matrix(0,9,3)
for(i in 1:9){
 cat_mo2021[i,3]=sum(YFS_catch$Weight.Posted..Sum.[which(YFS_catch$Year==2021&YFS_catch$Month==i)],na.rm=TRUE)
}
cat_mo2021[,2]=c("January","February","March","April","May","June","July","August","September")
colnames(cat_mo2021)=c("N","Month","Catch")
cat_mo2021=as.data.frame(cat_mo2021)
cat_mo2021$Month=factor(cat_mo2021$Month,levels=cat_mo2021$Month)
cat_mo2021$Catch=as.numeric(as.character(cat_mo2021$Catch))
Proportion=percent(round(cat_mo2021$Catch/sum(cat_mo2021$Catch),2))
cm2021=cbind(cat_mo2021,Proportion)

ggplot(cm2021, aes(x="", y=Catch, fill=Month))+
 geom_bar(width = 2, stat = "identity")+
 coord_polar("y", start=0,direction=-1)+
 blank_theme+theme(axis.text.x=element_blank()) +
 geom_text(aes(label = Proportion),position = position_stack(vjust =0.5),vjust=c(-.3,rep(1.1,8)),hjust=c(rep(.5,8),.3), size=c(5,5,5,5,5,5,3,5,5))+scale_fill_discrete(name = "Month")+ 
...
#table(length_file$NMFS.Area[which(length_file$Year==2021)])
lens=seq(7,58,1)#use 7-58cm
...
#  lensM[i,j]=nrow(length_file[which(length_file$Length..cm.==lens[i]&length_file$Sex=="M"&length_file$NMFS.Area==areas[j]&length_file$Gear<3&length_file$Year==2021),])
#  lensF[i,j]=nrow(length_file[which(length_file$Length..cm.==lens[i]&length_file$Sex=="F"&length_file$NMFS.Area==areas[j]&length_file$Gear<3&length_file$Year==2021),])
#   }
#}

#write.csv(lensM,"/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/lensM.csv")
#write.csv(lensF,"/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/lensF.csv")

lensM=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/lensM.csv",header=TRUE)
lensF=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/lensF.csv",header=TRUE)

lensM1=matrix(0,52,8);lensF1=lensM1
for(i in 2:8){
 lensM1[,i]=(lensM[,i]/sum(lensM[,i]))
  lensF1[,i]=(lensF[,i]/sum(lensF[,i]))
}

lensM2=cbind(rep(seq(7,58,1),length(areas)),rep(areas,each=length(lens)),as.vector(as.matrix(lensM1[,2:8])))
colnames(lensM2)=c("Length","Area","Frequency")

lensF2=cbind(rep(seq(7,58,1),length(areas)),rep(areas,each=length(lens)),as.vector(as.matrix(lensF1[,2:8])))
colnames(lensF2)=c("Length","Area","Frequency")

sc=data.frame(rbind(lensF2,lensM2))
sc=cbind(sc,c(rep("Females",nrow(lensF2)),rep("Males",nrow(lensM2))))
colnames(sc)[4]="Sex"
sc$Length=as.numeric(sc$Length)
sc$Frequency=as.numeric(sc$Frequency)
sc$Sex=as.factor(sc$Sex)
sc$Area=as.factor(sc$Area)

ggplot(sc[which(sc$Area!=517),])+geom_line(aes(x=Length,y=Frequency,group=Sex,color=Sex))+facet_wrap(~Area)+theme_bw()+labs(x="Length (cm)")+theme_bw()+
...
wts_f=YFS18_2$wt_srv_f
colnames(wts_f)=seq(1,20,1)
rownames(wts_f)=seq(1954,2021,1)

wts_m=YFS18_2$wt_srv_m
colnames(wts_m)=seq(1,20,1)
rownames(wts_m)=seq(1954,2021,1)

 d <- (as.data.frame(wts_f[47:68,]))
 d$Year=row.names(d)
...
Data=c(10*YFS18_2$Bottom_temp[1,1:37],(mal5[9:45]-mean(mal5[9:45])),(fem5[9:45]-mean(fem5[9:45])))       
...
ADFG=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/YellowfinNortonSound_ADFGsurvey.csv",header=TRUE)
...
```{r ricker1,echo=FALSE,message=FALSE}
 ...
 df <- data.frame(YFS2020$SRR_SSB)
 df$Year <- YFS2020$Year
 df$rhat <- YFS2020$rechat
 df$rhat.sd <- YFS2020$rechat.sd
 ...
 df2=data.frame(YFS2020$Yr[62:67],YFS2020$SSB[62:67,2],(YFS2020$natage_f[62:67,1]+YFS2020$natage_m[62:67,1]))
  df3=data.frame(YFS2020$Yr[25:61],YFS2020$SSB[25:61,2],(YFS2020$natage_f[25:61,1]+YFS2020$natage_m[25:61,1]))
...
```{r ricker2,echo=FALSE,message=FALSE}
 ...
 df$ub   <- df$rhat*exp(2*sqrt(log(1+df$rhat.sd^2/df$rhat^2)))
 colnames(df)=c("ssb","rhat","rhat.sd","lb","ub")
 df2=data.frame(YFS18_2$Yr[62:68],YFS18_2$SSB[62:68,2],(YFS18_2$natage_f[62:68,1]+YFS18_2$natage_m[62:68,1]))
  df3=data.frame(YFS18_2$Yr[25:62],YFS18_2$SSB[25:62,2],(YFS18_2$natage_f[25:62,1]+YFS18_2$natage_m[25:62,1]))
 colnames(df2)=c("Year","FSB","Recruitment")
  colnames(df3)=c("Year","FSB","Recruitment")
 ggplot(df) +geom_line(aes(x = ssb, y = rhat)) +
...
YFS2020=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m2020/fm.rep")#last years model
    ...
    df1 <- data.frame( Year    = YFS2020$yrs_srv )
    df1$Model <- "Model 18.2 (2020)"
...
```
`r figure_nums(name="plotq",caption = paste("Survey catchability for Model 18.2 (2020) and 18.2 (2021), 1982-",thisyr-1,".",sep=""))`
...
\pagebreak
```{r fishery_wtatage}
fshF=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/fishery_wtatage/matfsh_wtage2F_blanksfilled.csv",header=TRUE)
colnames(fshF)=c("Year",seq(2,20,1))
fshF2=melt(fshF,id=c("Year"))
colnames(fshF2)=c("Year","Age","Weight")
fF=ggplot(data=fshF2)+geom_point(aes(x=Age,y=Weight,color=Year))+geom_point(data=fshF2[which(fshF2$Year==2020),],aes(x=Age,y=Weight),color="red",size=6)+theme_bw()+ggtitle("Fishery weight at age - females (red circles = 2020)")

#1997 age 19 changed to previous year due to anomalous value.
#Age 19 males also changed to previous year due to no value.
fshM=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/fishery_wtatage/matfsh_wtage2M_blanksfilled.csv",header=TRUE)
colnames(fshM)=c("Year",seq(2,20,1))
fshM2=melt(fshM,id=c("Year"))
colnames(fshM2)=c("Year","Age","Weight")
fM=ggplot(data=fshM2)+geom_point(aes(x=Age,y=Weight,color=Year))+geom_point(data=fshM2[which(fshM2$Year==2020),],aes(x=Age,y=Weight),color="red",size=6)+theme_bw()+ggtitle("Fishery weight at age - males (red circles = 2020)")
...
```{r sexratio,message=FALSE,echo=FALSE}
        ...
        df1 <- data.frame(rowSums(YFS2020$natage_f)/rowSums(YFS2020$natage_f+YFS2020$natage_m))
...
nages=20
dff <- data.frame(Model="Model 18.2", sex="Females",cbind(YFS18_2$yrs_srv_age_s,YFS18_2$oac_srv_s[,1:nages]) )
dfm <- data.frame(Model="Model 18.2", sex="Males",cbind(YFS18_2$yrs_srv_age_s,YFS18_2$oac_srv_s[,(nages+1):(2*nages)]))
pff <- data.frame(Model="Model 18.2", sex="Females",cbind(YFS18_2$yrs_srv_age_s,YFS18_2$eac_srv_s[,1:nages]) )
pfm <- data.frame(Model="Model 18.2", sex="Males",cbind(YFS18_2$yrs_srv_age_s,YFS18_2$eac_srv_s[,(nages+1):(2*nages)]))
...
nages=20
dff <- data.frame(Model="Model 18.2", sex="Females",cbind(YFS18_2$yrs_fsh_age_s,YFS18_2$oac_fsh_s[,1:nages]) )
dfm <- data.frame(Model="Model 18.2", sex="Males",cbind(YFS18_2$yrs_fsh_age_s,YFS18_2$oac_fsh_s[,(nages+1):(2*nages)]))
pff <- data.frame(Model="Model 18.2", sex="Females",cbind(YFS18_2$yrs_fsh_age_s,YFS18_2$eac_fsh_s[,1:nages]) ) 
pfm <- data.frame(Model="Model 18.2", sex="Males",cbind(YFS18_2$yrs_fsh_age_s,YFS18_2$eac_fsh_s[,(nages+1):(2*nages)]))
colnames(dff) <- colnames(dfm) <- colnames(pff) <- colnames(pfm) <- c("Model", "Sex", "Year",as.character(1:nages) )
df <- rbind(dff,dfm)
pf <- rbind(pff,pfm)
mdf <- mpf <- mrf <- NULL
...
#fit to survey biomass models 18.2 and 18.2a
par(mar=c(0,0,0,0))
LCI=YFS18_2a$ob_bts-1.96*YFS18_2a$sd_ob_bts
HCI=YFS18_2a$ob_bts+1.96*YFS18_2a$sd_ob_bts
srv_bio1=data.frame(cbind(YFS18_2a$yrs_srv,YFS18_2a$ob_bts,YFS18_2a$sd_ob_bts,LCI,HCI,YFS18_2a$pred_srv[c(29:66,68)]))
colnames(srv_bio1)=c("Year","Biomass","sd","LCI","HCI","Predicted")

LCI=YFS18_2$ob_bts-1.96*YFS18_2$sd_ob_bts
HCI=YFS18_2$ob_bts+1.96*YFS18_2$sd_ob_bts
srv_bio2=data.frame(cbind(YFS18_2$yrs_srv,YFS18_2$ob_bts,YFS18_2$sd_ob_bts,LCI,HCI,YFS18_2$pred_srv[c(29:66,68)]))
colnames(srv_bio2)=c("Year","Biomass","sd","LCI","HCI","Predicted")


m1=ggplot(data=srv_bio2,aes(x=Year,y=Biomass))+geom_ribbon(aes(x=Year,y=Biomass,ymin=LCI,ymax=HCI),alpha=0.2)+geom_line(aes(x=Year,y=Predicted),col="blue")+geom_line(aes(x=Year,y=Biomass),col="black")+scale_x_continuous(breaks = seq(1982,2021, by = 2))+theme_bw()+ggtitle("Model 18.2, EBS design-based biomass estimate")


m2=ggplot(data=srv_bio1,aes(x=Year,y=Biomass))+geom_ribbon(aes(x=Year,y=Biomass,ymin=LCI,ymax=HCI),alpha=.2)+geom_line(aes(x=Year,y=Predicted),col="blue")+geom_line(aes(x=Year,y=Biomass),col="black")+scale_x_continuous(breaks = seq(1982,2022, by = 2))+theme_bw()+ggtitle("Model 18.2a, EBS VAST biomass estimate")

#fit to survey biomass models 18.4
LCI=YFS18_2b$ob_bts-1.96*YFS18_2b$sd_ob_bts
HCI=YFS18_2b$ob_bts+1.96*YFS18_2b$sd_ob_bts
srv_bio4=data.frame(cbind(YFS18_2b$yrs_srv,YFS18_2b$ob_bts,YFS18_2b$sd_ob_bts,LCI,HCI,YFS18_2b$pred_srv[c(29:66,68)]))
Model=c(rep("Model 18.2b",nrow(srv_bio4)))
...
```
`r figure_nums(name="srvbio_fit",caption = paste("Model 18.2 (2021) (upper panel), Model 18.2a (middle panel), and Model 18.2b (lower panel) fit to NMFS eastern Bering Sea survey biomass estimates, from 1982-",thisyr-1,". Models 18.2, and 18.3a incorporate estimates from the EBS only, while Model 18.2b used NBS+EBS estimates. Models 18.2a and 18.2b used VAST biomass and standard errror, while Model 18.2 used design-based biomass and error estimates.",sep=""))`
...
TotBiom18_2_2020=data.frame(YFS2020$TotBiom,rep("18.2_2020",nrow(YFS2020$TotBiom)))
TotBiom18_2=data.frame(YFS18_2$TotBiom,rep("18.2",nrow(YFS18_2$TotBiom)))
TotBiom18_2a=data.frame(YFS18_2a$TotBiom,rep("18.2a",nrow(YFS18_2a$TotBiom)))
TotBiom18_2b=data.frame(YFS18_2b$TotBiom,rep("18.2b",nrow(YFS18_2b$TotBiom)))
colnames(TotBiom18_2_2020)=c("Year","Biomass","sd","LCI","HCI","Model")
colnames(TotBiom18_2)=c("Year","Biomass","sd","LCI","HCI","Model")
colnames(TotBiom18_2a)=c("Year","Biomass","sd","LCI","HCI","Model")
colnames(TotBiom18_2b)=c("Year","Biomass","sd","LCI","HCI","Model")
TotBiom=rbind(TotBiom18_2_2020,TotBiom18_2,TotBiom18_2a,TotBiom18_2b)
...
SSBiom18_2_2020=data.frame(YFS2020$SSB,rep("18.2_2020",nrow(YFS2020$SSB)))
SSBiom18_2=data.frame(YFS18_2$SSB,rep("18.2",nrow(YFS18_2$SSB)))
SSBiom18_2a=data.frame(YFS18_2a$SSB,rep("18.2a",nrow(YFS18_2a$SSB)))
SSBiom18_2b=data.frame(YFS18_2b$SSB,rep("18.2b",nrow(YFS18_2b$SSB)))
colnames(SSBiom18_2_2020)=c("Year","SSB","sd","LCI","HCI","Model")
colnames(SSBiom18_2)=c("Year","SSB","sd","LCI","HCI","Model")
colnames(SSBiom18_2a)=c("Year","SSB","sd","LCI","HCI","Model")
colnames(SSBiom18_2b)=c("Year","SSB","sd","LCI","HCI","Model")
SSBiom=rbind(SSBiom18_2_2020,SSBiom18_2,SSBiom18_2a,SSBiom18_2b)
...
`r figure_nums(name="BIOM_SSB",caption = paste("Total (age 2+) and spawning stock biomass for Yellowfin Sole, based on Models 18.2 (2020), 18.2, 18.2a, and 18.2b, from 1954-2021."))`


\pagebreak
```{r bioFSBCI, message=FALSE, echo=FALSE} 
TotBiom=data.frame(YFS18_2$TotBiom,rep("Biomass",nrow(YFS18_2$TotBiom)))
colnames(TotBiom)=c("Year","Biomass","sd","LCI","HCI","Type")
SSB=data.frame(YFS18_2$SSB,rep("FSB",nrow(YFS18_2$SSB)))
...
pcatch=read.table("/Users/ingridspies/admbmodels/projection/bsai_yfin2021_out/percentiles.out",nrows=14,skip=144)
colnames(pcatch)=c("Year","C0","Cabc","Cofl","LowCI_Catch ","Median_Catch","Mean_Catch","UpperCI_Catch","Stdev_Catch")

pSSB=read.table("/Users/ingridspies/admbmodels/projection/bsai_yfin2021_out/percentiles.out",nrows=14,skip=161)
...
`r figure_nums(name="fishingavgF",caption=paste("Projected female spawning biomass for ",thisyr," to ",thisyr+13," (blue line), with 5% and 95% confidence intervals, and fishing at the 5-year (2016-2020) average fishing mortality rate, F=",round(mean(YFS18_2$F_m[63:67,18]),4),", Model 18.2."))`

\pagebreak
```{r recfig,echo=FALSE,message=FALSE}

recf=data.frame(YFS18_2$Yr[1:63],YFS18_2$natage_f[1:63,5]+YFS18_2$natage_m[1:63,5])
colnames(recf)=c("Year","Recruitment")
ggplot(data=recf)+geom_bar(aes(x=Year,y=Recruitment),alpha=0.4,stat="identity")+geom_hline(yintercept=mean(YFS18_2$natage_f[1:63,5]+YFS18_2$natage_m[1:63,5]))+theme_bw()+scale_x_continuous(breaks = seq(1954,2016, by = 2))+theme(axis.text.x=element_text(angle=45,hjust=1))+ylab("Recruitment, age 5, billions of fish")
```
`r figure_nums(name="recfig",caption=paste("Year-class strength of age 5 Yellowfin Sole estimated by the stock assessment model. The horizontal line represents the average of the estimates from recruitment, 1954-2016, 1.78 billion, Model 18.2."))`
...
#retro 18.2
std2021=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_0/fm.std")
std2020=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_1/fm.std")
std2019=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_2/fm.std")
std2018=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_3/fm.std")
std2017=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_4/fm.std")
std2016=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_5/fm.std")
std2015=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_6/fm.std")
std2014=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_7/fm.std")
std2013=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_8/fm.std")
std2012=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_9/fm.std")
std2011=read.table("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/retro18_2/retro_10/fm.std")
...
retro=as.data.frame(rbind(ret11,ret12,ret13,ret14,ret15,ret16,ret17,ret18,ret19,ret20,ret21))
colnames(retro)=c("Year","Female_Spawning_Biomass","UI","LI")

year=rev(c(rep("2021",length(c(1954:2021))),rep("2020",length(c(1954:2020))),rep("2019",length(c(1954:2019))),rep("2018",length(c(1954:2018))),rep("2017",length(c(1954:2017))),rep("2016",length(c(1954:2016))),rep("2015",length(c(1954:2015))),rep("2014",length(c(1954:2014))),rep("2013",length(c(1954:2013))),rep("2012",length(c(1954:2012))),rep("2011",length(c(1954:2011)))))
...
#plot differences
Years=c(c(1954:2020),c(1954:2019),c(1954:2018),c(1954:2017),c(1954:2016),c(1954:2015),c(1954:2014),c(1954:2013),c(1954:2012),c(1954:2011))


Difference=c(-1*(YFS2021$SSB[1:67,2]-YFS2020$SSB[,2])/YFS2021$SSB[1:67,2],
             -1*(YFS2020$SSB[1:66,2]-YFS2019$SSB[,2])/YFS2020$SSB[1:66,2],
...
Year=(c(rep("2020",length(c(1954:2020))),rep("2019",length(c(1954:2019))),rep("2018",length(c(1954:2018))),rep("2017",length(c(1954:2017))),rep("2016",length(c(1954:2016))),rep("2015",length(c(1954:2015))),rep("2014",length(c(1954:2014))),rep("2013",length(c(1954:2013))),rep("2012",length(c(1954:2012))),rep("2011",length(c(1954:2011)))))
...
p18_2b=ggplot(data=dat[which(dat$Years>1974),],aes(x=Years,y=Difference,type=Year))+geom_line(aes(x=Years,y=Difference, color=Year))+ylab("Difference")+xlab("Year")+theme_bw()+theme(legend.text=element_text(size=11),axis.text=element_text(size=9),axis.title.x = element_text(size=14),axis.title.y = element_text(size=14))+scale_x_continuous(breaks = seq(1974,2020, by = 2))+theme(axis.text.x=element_text(angle=90,hjust=-1))


p18_2+ggtitle("Model 18.2")
p18_2b+ggtitle("Model 18.2, 1974-2020")
```
...
par(mar=c(5,5,3,2))
par(mfrow=c(1,1))
F40=ABC_OFL18_2$HM_Fmsyr[1]
B40=ABC_OFL18_2$Bmsy[1]
FOFL=ABC_OFL18_2$AM_Fmsyr[1]
plot(0,0,xlim=c(0,max(YFS18_2$SSB[,2])*1.1),ylim=c(0,FOFL*1.6),col=1,xlab="Estimated female spawning biomass (x 1,000 t)",ylab="Estimated fishing mortality rate",cex.axis=1.2,cex.lab=1.1,pch=-1,cex=.00001)
segments(0,0,B40,F40,col="green",lwd=4)
segments(B40,F40,max(YFS18_2$SSB[,2])*1.1,F40,col="green",lwd=4)
abline(h=FOFL,lty=1)
abline(v=B40,lty=2)
...
points(YFS18_2$SSB[21:68,2],YFS18_2$F_f[21:68,19],type="l",col="red",lwd=4)

points(YFS18_2$SSB[68,2],YFS18_2$F_f[68,19],pch=15,col="red")
text(YFS18_2$SSB[68,2]-60,YFS18_2$F_f[68,19]+.005,"2021",cex=0.9)  

#for(i in 1: 59){
#text(ATF2019$fspbio[i]  ,ATF2019$fishmort[i],labels=as.character(seq(1961,2019,1))[i],cex=0.9)}

points(ABC_OFL18_2$SSB[1],ABC_OFL18_2$Catch_Assump[1]/ABC_OFL18_2$GM_Biom[1],pch=15,col="blue")
text(ABC_OFL18_2$SSB[1],(ABC_OFL18_2$Catch_Assump[1]/ABC_OFL18_2$GM_Biom[1])+.005,"2022",cex=0.9)

points(ABC_OFL18_2$SSB[2]+20,ABC_OFL18_2$Catch_Assump[2]/ABC_OFL18_2$GM_Biom[2],pch=15,col="black")
text(ABC_OFL18_2$SSB[2]-60,(ABC_OFL18_2$Catch_Assump[2]/ABC_OFL18_2$GM_Biom[2])+.005,"2023",cex=0.9) 


```

`r figure_nums(name="phaseplane",caption = paste("Fishing mortality rate and female spawning biomass from 1975 to ",thisyr," compared to the F35% and F40% control rules, based on Model 18.2.  Vertical line is B35%. Squares indicate estimates for 2021, 2022, and 2023.",sep=""))`

\pagebreak

## Appendix A

Table A1. Removals (kg) of Yellowfin Sole from the Bering Sea from sources other than those that are included in the Alaska Region’s official estimate of catch, 2006-2019. Source NMFS Alaska Region: Sourced by the AKR.V_NONCOMMERCIAL_FISHERY_CATCH table, October 23, `r thisyr`. Abbreviations: IPHC (International Pacific Halibut Commission), ADFG (Alaska Department of Fish and Game), NMFS (National Marine Fisheries Service).
...
recatch2=read.csv("/Users/ingridspies/Documents/WorkDellStuff/Assessments/YFS/2021/NoncommercialFisheryCatch_YFS_10_14_21.csv",header=TRUE)
...
fisselT1=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/Fissel_T1.csv")
...
fisselT2=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/2020/Fissel_T2.csv")
