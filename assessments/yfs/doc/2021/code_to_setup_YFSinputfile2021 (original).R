...
for(j in 1:length(N)){
 for(i in 1:N[j]){
  ...
  tempMAT_len[j,i]=tempz[which(tempz$YEAR==1981+i+j-1),3]
 }}
...
ggplot(data=OUT_len2)+geom_point(aes(x=Age1,y=Growth_Male,color=Temperature))+theme_bw()+ggtitle("Male Growth by Age")#Year implies year of birth only

ggplot(data=OUT_len2)+geom_point(aes(x=Age1,y=Growth_Female,color=Temperature))+theme_bw()+ggtitle("Female Growth by Age")#Year implies year of birth only

#Growth by length for all ages and temperature (year too but year corelates with temp)

ggplot(data=OUT_len2)+geom_point(aes(x=Temperature,y=Growth_Male,color=Year))+theme_bw()+ggtitle("Male Growth by increment and Temperature all ages")#+xlim(c(.1,5.5)) #yes

#Is it possible that more recent years grow faster at the same temperatures? It sort of looks like it here
ggplot(data=OUT_len2)+geom_point(aes(x=Temperature,y=Growth_Female,color=Year))+theme_bw()+ggtitle("YFS Female Growth by increment and Temperature all ages")
...
leveneTest(Growth_Female~Age_factor,OUT_len)#not significant
leveneTest(Growth_Male~Age_factor,OUT_len)#Not significant for males 
...
leveneTest(Growth_Female~Temp_factor,OUT_len2)#no significant difference in variance by temperature once temp=0 is removed. This is good
leveneTest(Growth_Male~Temp_factor,OUT_len2)#For males there seems to be difference in variance by temperature (?).

fit = aov(Growth_Female ~ Temperature, OUT_len2)#Effect for females with all ages
summary(fit)#Females seem to have a temperature effect. 
fit = aov(Growth_Male ~ Temperature, OUT_len2)#No significant effect foe males with all ages.
summary(fit)#Males no effect on growth by temperature
fit = aov(Growth_Male ~ Temperature, OUT_len2[which(OUT_len2$Age1<7),])#No significant effect fro males with age 1-8
...
ggplot(OUT_len2,aes(y=Growth_Female, x=Temp_factor, fill=Temp_factor))+
...
ggplot(OUT_len2,aes(y=Growth_Male, x=Temp_factor, fill=Temp_factor))+
 stat_summary(fun="mean", geom="bar",position="dodge")+
...
fit2=aov(Growth_Female~Age1+Temperature,OUT_len2)
Anova(fit2, type="III")
#Females controlling for any effects of age, Temperature seems to affect growth

fit2=aov(Growth_Male~Temperature+Age1,OUT_len2)
Anova(fit2, type="III")
#Males no effect on growth by temperature

#Can I test the effect on growth by year controlling for temperature?
fit2=aov(Growth_Female~Year+Temperature,OUT_len2)
Anova(fit2, type="III")
#Seems to be a year effect if you control for temperature

fit2=aov(Growth_Male~Year+Temperature,OUT_len2)
Anova(fit2, type="III")
#Again with males, seems to be a year effect if you control for temperature
