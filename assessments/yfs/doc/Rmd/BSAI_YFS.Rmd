...
knitr::opts_chunk$set(setwd("/Users/ingridspies/admbtest/YFS/YFS/"))
...
extra_sd=read.table("./assessments/yfs/runs/m1/extra_sd.rep") 
...
res_mat2=cbind(as.numeric(as.character(yr)),formatC(cat,format="d",big.mark="m"))
...
Total mortality $Z$ in the model is the sum of fishing mortality $F$ and natural mortality $M$, such that total mortality in year $t$ at age $a$ is  $Z_{t,a}=F_{t,a}+M$. Fishing mortality at each year and age $F_{t,a}$ is the product of age-specific fishing gear selectivity $s_a$ and the median year-effect of fishing mortality ${\mu}^F$, with normally distributed error, $F_{t,a}=s_a{{\mu}^F}e^{{\epsilon}^F_t}, {{\epsilon}^F_t}\backsim N(0,{{\sigma}_F^2})$, where ${\epsilon}^F_t$ is the residual year-effect of fishing mortality and ${\sigma}_F$ is the standard deviation of the residual year-effect of fishing mortality. 

Recruitment 1956-1975:  $N_{t,1}=R_{t}=R_{0}e^{\tau_{t}}, \tau \backsim\ N(0,\delta^2_R)$, where $R_0$ is the geometric mean of age 1 recruitment, 1956-1975.

Recruitment 1976-`r thisyr` was determined using the Ricker stock recruitment curve, $R={\alpha}Se^{-{\beta}S}$, with parameters determined using data from the period 1976-`r thisyr-6`.

$N_{t,1}=R_t=R_{\gamma}e^{\tau_{t}},$ where $R_\gamma$ is the geometric mean value of age 1 recruitment, 1976-2014.

Catch in year $t$ for age $a$ fish:  $C_{t,a}={\displaystyle\frac{F_{t,a}}{Z_{t,a}}}(1-e^{Z_{t,a}})N_{t,a},$ where $C$ is catch, $F$ is instantaneous annual fishing mortality, $Z$ is instantaneous total mortality, and $N$ is the number of fish.

Number of fish in year $t+1$ at age $a$: $N_{t+1,a+1}=N_{t,a}e^{-Z_{t,a}}$.

Number of fish in the "plus group": $N_{t+1,A}=N_{t,a}e^{-Z_{t,A-1}}+N_{t,A}e^{-Z_{t,A}}$.

Spawning biomass: $S_t={\sum}N_{t,a}W_{t,a}{\phi}_a$, where ${\phi}_a$ is the proportion of mature females at age $a$ and $W_{a,t}$ is the mean body weight in kg of fish age $a$ in year $t$.



The age-specific fishing selectivity 

$Length={\displaystyle\frac{S_{inf}}{1+e^{-K*(age-t_0))}}}.$
