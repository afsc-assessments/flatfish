...
YFS=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m1/fm.rep")
...
B0=formatC(1000*YFS2$Bzero,format="d",big.mark=",")
```
...
1. The model used in the `r thisyr-1` is referred to as Model 18.1. 
...
Tier	&	1a	&	1a	&	1a	&	1a \\
Projected total (age 6+) biomass (t)	& 2,388,000 t & 2,331,500 t &	`r formatC(extra_sd2[1,4]*1000,format="d",big.mark=",")` t	   &	`r formatC(extra_sd2[2,4]*1000,format="d",big.mark=",")` t  \\
...
$\:\:\:\:\:\:B_{100\%}$                                 &	1,236,000 t	&	1,236,000 t	&		`r B0` t	       &	`r B0` t    \\
...
A new base model (18.1) and several variants were introduced in 2018, motivated in part by an unexpected 32% decrease in the survey biomass. Model 18.1 includes the survey mean bottom temperature across stations < 100m as a covariate on survey catchability, as in the old model, but adds survey start date as an additional covariate within the model, based on a recent study by Nichol et al. (2018). Natural mortality remains fixed at 0.12 for both sexes in the new model. The model resulted in substantial improvements to the likelihood and AIC, and both the author and PT recommend model 18.1. The SSC agreed with the choice of model 18.1 to set the ABC and OFL for 2019/2020.
...
Wilderbuer, T.K., G.E. Walters, and R.G. Bakkala   1992.  Yellowfin sole, Pleuronectes asper, of the eastern Bering Sea: biological characteristics, history of exploitation, and management.  Mar Fish. Rev. 54(4):1-18.
...
a=data.frame(round(YFS2$catage_f,4)[22:(nrow(YFS2$catage_f)-1),7:17])
...
totallike1=sum(YFS$wt_like,YFS$age_likeihood_for_survey,YFS$age_likelihood_for_fishery,YFS$catch_likelihood,YFS$selectivity_likelihood,YFS$survey_likelihood,YFS$recruitment_likelilhood)  
```
...
\hline
Survey age	        &       `r round(YFS$age_likeihood_for_survey,2)`  &  `r round(YFS2$age_likeihood_for_survey,2)`   \\
Fishery age  	  &       `r round(YFS$age_likelihood_for_fishery,2)` &  `r round(YFS2$age_likelihood_for_fishery,2)`  \\
Selectivity     &       `r round(sum(YFS$selectivity_likelihood),2)`& `r round(sum(YFS2$selectivity_likelihood),2)`\\
Survey biomass     &       `r round(YFS$survey_likelihood,2)`& `r round(YFS2$survey_likelihood,2)`\\
Recruitment     &       `r round(sum(YFS$recruitment_likelilhood),2)`& `r round(sum(YFS2$recruitment_likelilhood),2)`  \\
\hline
...
`r table_nums('tab_9')`
```{r tab_9}
wt_f=YFS2$wt_srv_f[11:65,]
wt_m=YFS2$wt_srv_m[11:65,]
