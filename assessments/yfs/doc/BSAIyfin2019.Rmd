...
YFS1=read.rep("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/runs/m2/fm.rep")
...
For this assessment, two models were examined. The first was the same model used in the 2018 assessment, Model 18.1a. The second model, Model 18.2, fixed female natural mortality at $M$=0.12 as in previous years, but allowed the model to freely estimate male natural mortality.

The trend in survey catchability was similar with Model 18.1a and Model 18.2, but the catchability was lower with Model 18.2 and estimates in some years were slightly over 1.0 (`r figure_nums("plotq",display="cite")`). 

The sex ratio, presented as the proportion female, was estimated to be slightly lower in Model 18.1a than Model 18.2 (`r figure_nums("sexratio",display="cite")`). Overall, the total negative log likelihod was lower for Model 18.2, and provided a better fit to the survey and fishery ages, as well as an improvement to the fit to survey catchability, with the total negative log likelihood reduced from 1,424 in Model 18.1a to 1,356 in Model 18.2 (`r table_nums("liketab",display="cite")`, figure_nums("srv_agecomp1",display="cite")`, figure_nums("srv_agecomp2",display="cite")`,  figure_nums("fsh_agecomp1",display="cite")`,  figure_nums("fsh_agecomp2",display="cite")`). 

`r table_nums("Mcomparison",display="cite")` indicates that the ABC values from Model 18.2 for `r thisyr+1` would be `r formatC(1000*(extra_sd2$ABC_HM[1]-extra_sd1$ABC_HM[1]),format="d",big.mark=",")` t higher than Model 18.2. This is due to the higher biomass estimate resulting from an increased value of male natural mortality in Model 18.2. Model 18.2 also provided a slightly better fit to survey biomass, but this effect was primarily noticeable during the years 1988-1995 . Overall Model 18.2 provided very little change in the fit to survey biomass (`r figure_nums("srvbio_fit",display="cite")`). 
...
Posterior distributions of several key parameters in the model capture variability in posterior distributions of parameter estimates and differences between Model 18.2 and Model 18.1a (`r figure_nums("mcmcplots",display="cite")`). Model 18.2 resulted in higher estimates for $B_{MSY}$, biomass and female spawning biomass and recruitment, but similar values to Model 18.1a for $F_{MSY}$. The posterior distribution for female spawning biomass is above the Model 18.2 estimate for $B_{MSY}$ (`r figure_nums("mcmcplots",display="cite")`).
...
Simulation results shown in `r table_nums("projection",display="cite")` indicate that Yellowfin Sole are not currently overfished and are not approaching an overfished condition.  A phase plane figure of the estimated time-series of Yellowfin Sole female spawning biomass (FSB) relative to the harvest control rule is shown in `r figure_nums("phaseplane",display="cite")`.
...
kable(rd1[,2:4],"latex",booktabs=T,linesep="",longtable=FALSE,align="r")
```


\pagebreak
`r table_nums(name = "wtagetab", caption = "Mean weight at age for Yellowfin Sole (unsmoothed), females presented first, followed by males.")`
```{r wtagetab}
thisyr=2019
...
\pagebreak
`r table_nums(name = "Srvbio_CI", caption = "Yellowfin Sole biomass estimates (t) from the annual Bering Sea shelf bottom trawl survey, with upper and lower 95% confidence intervals.")`
...
\pagebreak
`r table_nums(name = "FSBCI", caption = paste("Model estimates of Yellowfin Sole female spawning biomass (FSB) in metric tons (t) and upper (HCI) and lower (LCI) 95% confidence intervals from the",thisyr-1," and ",thisyr," stock assessments, including Model 18.1a and 18.2."))`
...
\pagebreak
`r table_nums(name="rec",caption="Model estimates of age 5 recruitment (in billions of fish), 1954-2019, with 95% lower and upper confidence intervals (LCI, HCI).")`
```{r rec, message=FALSE,echo=FALSE}

rec=data.frame(cbind(YFS2$rechat,YFS2$rechat-1.96*YFS2$rechat.sd,YFS2$rechat+1.96*YFS2$rechat.sd,YFS1$rechat,YFS1$rechat-1.96*YFS1$rechat.sd,YFS1$rechat+1.96*YFS2$rechat.sd))

rownames(rec)=seq(1989,2019,1)
colnames(rec)=c("Recruitment","LCI","HCI","Recruitment","LCI","HCI")
kable(rec,"latex",booktabs=TRUE,linesep="",align="r",digits=3)%>%add_header_above(c(" "=1,"Model 18.2"=3,"Model 18.1a"=3))

```


\pagebreak
`r table_nums(name = "ABCOFL", caption = paste("Yellowfin sole TAC, OFL, and ABC levels, 1980-2019. Data is in metric tons."))`
```{r ABCOFL, message=FALSE,echo=FALSE}
ABCOFL=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/YFS_ABCOFL.csv",header=TRUE)


colnames(ABCOFL)=c("Year","TAC","OFL","Catch")
...
#bycatch1=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/FMP Groundfish BycatchYFIN.csv",header=TRUE)
...
#colnames(b1f)=spp
#rownames(b1f)=yrs
#write.csv(b1f,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/b1f.csv")
b1fa=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/b1fa.csv",header=TRUE)
...
b1fb=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/b1fb.csv",header=TRUE)
...
bycatch2=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/FMP Other Species Bycatch CAS2_YFIN.csv",header=TRUE)
...
#bycatch3=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/Nontarget Species BycatchYFIN.csv",header=TRUE)
...
#colnames(b3f)=spp
#rownames(b3f)=yrs
#write.csv(b3f,"/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/b3f.csv")
b3f=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/b3f.csv",header=TRUE)
...
ebsl=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/EBS Shelf YFSlens.csv",header=TRUE)
...
Males=c(ebsltab[,2],ebsltab[,4])
Females=c(ebsltab[,3],ebsltab[,5])
...
nbsl=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/NBS_YFS_lens.csv",header=TRUE)
...
\begin{center}
\includegraphics[width=5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/Yellowfin Sole Spawning Distribution-B.pdf}
\end{center}
...
```
`r figure_nums(name='tempanom',caption="Yellowfin sole length-at-age anomalies, for 5-year old males and females, and bottom temperature anomalies.  Correspondence in these residuals is apparent with a 2-3 year lag effect from the mid-1990s to 2019.  Late 1980s and early 1990s pattern may be a density-dependent response in growth from the large 1981 and 1983 year-classes. Note: Bottom temperature anomalies were scaled up by a factor of 10 to demonstrate the pattern and match length anomalies.")`
...
sc=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/2019YFSfisherylengths.csv",header=TRUE)
sc1=sc/colSums(sc)
...
cat_mo2019b=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/cat_mo2019b.csv")
...
```
\begin{center}
\includegraphics[width=7.5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/CPUE_2019bymo2.png}
\end{center}
...
cpue=read.csv("/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/race_cpue_by_haul.csv",header=TRUE)
...
```
\begin{center}
\includegraphics[width=7.5in,angle=0,origin=c]{/Users/ingridspies/admbmodels/flatfish/assessments/yfs/doc/data/CPUE_YFS_allyrs.png}
\end{center}

`r figure_nums(name='cpuefig',caption="Catch of Yellowfin Sole in the BSAI, 1991-2019. Circles represent relative catch in ADFG Statistical Areas.")`
...
\end{center}
`r figure_nums(name="cpuediff",caption = "Difference between the 1985-2018 average trawl survey CPUE for Yellowfin Sole and the 2019 survey CPUE.  Red circles indicate that the magnitude of the catch was greater in 2019 than the long-term average, green circles indicate the catch was lower in the long-term average than in 2019.")`
...
```{r ricker1,echo=FALSE,message=FALSE}
 ...
 ggplot(df) +geom_line(aes(x = ssb, y = rhat)) +
  geom_ribbon(aes(x = ssb, ymax = ub, ymin = lb), alpha=0.3)+labs(x = "Female spawning biomass (x 1,000 t)", y = "Recruits (age 1, millions)")+geom_text(data=df2,aes(x = FSB, y = Recruitment,label=Year),color="blue")+geom_text(data=df3,aes(x = FSB, y = Recruitment,label=Year))+ggtitle("Model 18.1a")+theme_bw()
...
```{r ricker2,echo=FALSE,message=FALSE}
 ...
 colnames(df2)=c("Year","FSB","Recruitment")
  colnames(df3)=c("Year","FSB","Recruitment")
 ggplot(df) +geom_line(aes(x = ssb, y = rhat)) +
  geom_ribbon(aes(x = ssb, ymax = ub, ymin = lb), alpha=0.3)+labs(x = "Female spawning biomass (x 1,000 t)", y = "Recruits (age 1, millions)")+geom_text(data=df2,aes(x = FSB, y = Recruitment,label=Year),color="blue")+geom_text(data=df3,aes(x = FSB, y = Recruitment,label=Year))+ggtitle("Model 18.2")+theme_bw()
...
\pagebreak
```{r fishsel,fig.height=8.5,echo=FALSE,message=FALSE}
...
`r figure_nums(name="fsh_agecomp1",caption=paste("Model 18.2 fit to the time-series of fishery age composition, by sex, 1975-",thisyr-1,".",sep=""))`
...
```
`r figure_nums(name="bioFSBCI",caption = paste("Model estimates of total (age 2+) and female spawning biomass with 95% confidence intervals, 1954-",thisyr,".",sep=""))`
...
`r figure_nums(name="fishingavgF",caption=paste("Projected female spawning biomass for ",thisyr," to ",thisyr+13," to 2032 (blue line), with 5% and 95% confidence intervals, and fishing at the 5-year (2014-2018) average fishing mortality rate, F=",round(mean(YFS2$F_m[61:65,18]),4),"."))`
...
```
`r figure_nums(name="recfig",caption=paste("Year class strength of age 5 yellowfin sole estimated by the stock assessment model. The dotted line is the average of the estimates from 63 years of recruitment."))`
...
`r figure_nums(name="phaseplane",caption = paste("Fishing mortality rate and female spawning biomass from 1975 to ",thisyr," compared to the F35% and F40% control rules.  Vertical lines are B35% and B40%.",sep=""))`
