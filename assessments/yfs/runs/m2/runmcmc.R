...
m <- 'fm'
d <- './mcmc' # the model name, folder is also assumed to be called this runs/base/mcmc
## First optimize the model to make sure the Hessian is good.
setwd(d); 
system('fm -nox -mcmc 15 -hbf 1 -ainp fm.par -phase 50'); 
setwd('..')
...
?sample_admb
setwd(d); 
 qqapilot <- sample_admb(m, iter=iter, thin=thin, seeds=seeds, init=inits,
                     parallel=TRUE, chains=reps, warmup=warmup,
                     cores=reps, algorithm='RWM')
                     #path=getwd(), cores=reps, algorithm='RWM')
...
nuts.mle <-
  sample_admb(model=m, iter=500, init=NULL, algorithm='NUTS', thin=2,  seeds=seeds,
