...
.get_like_df <- function(M)
{
    ...
    for (i in 1:n)
    {
        ...
        df$obj_fun <- A$obj_fun 
        df$Survey <- A$survey_likelihood 
        df$rmse  <- A$rmse
        df$Catch  <- A$catch_likelihood 
        df$Age_fsh  <- A$age_likelihood_for_fishery
        df$Age_srv  <- A$age_likeihood_for_survey 
        df$rec      <- sum(A$recruitment_likelilhood)
        df$select  <- sum(A$selectivity_likelihood )
        df$q_prior  <- A$q_Prior 
        df$m_prior  <- A$m_Prior 
        df$F_pen  <- A$F_penalty
        mdf      <- rbind(mdf, df)
    }
    return(mdf)
}
