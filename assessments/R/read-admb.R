#' write ADMB input files
#'
#' @author Jim Ianelli
#' @param D list of stuff to write
#' @param fn file name to be written
#' @param hdr name to be written at top of file (as comment)
#' @return NA
#' @export
#' 
write_dat <- function(D,fn,hdr="a new file") {
    # The following writes a data file
    cat(file=fn,paste0("# ",hdr,"\n"))
    ol <-length(D)
    for (i in 1:ol){
      cat(file=fn,paste0("#",names(D[i]),"\n"),append=TRUE)
      write.table(D[[i]],file=fn,append=TRUE,row.names=FALSE,col.names=FALSE)
    }
}
...
read_dat <- function(fn) {
    ...
    # Identify strings w/o # in name
    idx=sapply(as.double(ifile),is.na)
    #idx2 = !substr(ifile[idx],1,1)=="#"
    #ifile[idx2]
