rm(list=ls())
source("../R/prelims.R")
mqtext <- as.matrix(read.table("M_q.dat",as.is=TRUE))
...
for (i in 1:30) {
  for (j in 0:10) {
    ...
    btmp$M <- rtmp$M <- Mspec[i]
    btmp$q <- rtmp$q <- qspec[i]
    ssb <- rbind(ssb,btmp)
    rec <- rbind(rec,rtmp)
  }
}
...
for (j in ssb$mq) {
  ...
  for (i in 1:10) {
    ...
    rho = rho + (-(dtmp2[i,2]-rc[i]))/rc[lr]
  }    
  rho.df <- rbind(rho.df,da)
}    
names(dtmp)
dtmp$Term_yr <- as.factor(dtmp$termyr)
ggplot(dtmp,aes(x=Year,y=SSB,color=Term_yr)) + geom_line(size=2) + xlim(c(1995,2017))+ .THEME
...
modset2 <- list( Bholt=mod6, Bholt_Prio1Gr_only=mod7)
